...
// Revisions
//
// 2014-01-12 SG: Reorganized.
// 2014-04-14 SG: Improvements for watchdog timer.
// 2014-06-14 SG: Moved icon URLs to neuro.map.
// 2014-07-01 SG: Fix bug caused by race condition.
// 2019-02-19 SG: Trapped bad GPS coordinates, seen when using desktop IE browser. Should not have been seen in actual mobile devices, but let's be careful.
...
neuro.GeoPos = {
    ...
    GET_POSITION_TIMEOUT_MS: 12000,     // Timeout used for navigator.geolocation methods. Some phones are rather slow to get GPS position.

    // Constatnts for error codes.

    ERROR_CODE_INVALID_COORDINATES: -2, // Our internal code for invalid coordinates from desktop browser
    ERROR_CODE_WATCHDOG_TIMEOUT: -1,    // Our internal code for watchdog timeout
    ERROR_CODE_OK: 0,                   // All's well
    ERROR_CODE_DENIED_BY_USER: 1,       // User denied permission for location services
    ERROR_CODE_GPS_NOT_AVAILABLE: 2,    // GPS position is not available to the device
    ERROR_CODE_GPS_TIMEOUT: 3,          // Device timed out getting GPS location
    ...
    WATCHDOG_TIMEOUT_ERROR_CODE: this.ERROR_CODE_WATCHDOG_TIMEOUT,  // Error code for our own watchdog
...
neuro.GeoPos.GeoData.prototype.positionErrorMessage = function () {
    ...
    switch (this.positionErrorCode) {
        ...
        //case 0: message = 'GPS Ok'; break; // No error
        //case 1: message = 'You denied permission for GPS'; break;
        //case 2: message = 'GPS position is not available'; break;
        //case 3: message = 'Timed out waiting for GPS'; break;

        case neuro.GeoPos.ERROR_CODE_INVALID_COORDINATES: message = 'Invalid GPS coordinates'; break; // Our internal error from browser returning NaN coordinates
        case neuro.GeoPos.ERROR_CODE_WATCHDOG_TIMEOUT: message = 'GPS device timeout'; break; // Our internal timeout
        case neuro.GeoPos.ERROR_CODE_OK: message = 'GPS Ok'; break; // No error
        case neuro.GeoPos.ERROR_CODE_DENIED_BY_USER: message = 'You denied permission for GPS'; break;
        case neuro.GeoPos.ERROR_CODE_GPS_NOT_AVAILABLE: message = 'GPS position is not available'; break;
        case neuro.GeoPos.ERROR_CODE_GPS_TIMEOUT: message = 'Timed out waiting for GPS'; break;
        default: message = 'Unknown GPS error code ' + this.positionErrorCode; break;
    }
...
neuro.GeoPos.GeoData.prototype.getPosition = function (parentCallback) {
    ...
    // 7/1/14 bug fixed. Code below caused a race condition on slower mobile devices
...
neuro.GeoPos.GeoData.prototype.getPositionSuccessCallBack = function (position) {
    ...
    // 2019-02-19 SG: If the coordinates are bad, treat this as an error, even though this is a 
    // "successful" callback. We've only seen this when testing with the IE desktop browser.
    ...
    else { // Treat this like an error

        error = {};
        error.code = neuro.GeoPos.ERROR_CODE_INVALID_COORDINATES;
        neuro.GeoPos.GeoData.prototype.getPositionFailCallBack(error);
    }
};
