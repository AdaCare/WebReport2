...
//
// adacare.lib.fieldfuncs.v1.07.js
// Sandy Gettings
//
...
//
// Revisions:
// 2014-06-25 SG: Added handler for checkbox fields.
// 2018-07-05 SG: Change phone number formatting to be stricter for US.
// 2018-07-22 SG: Remove any leading "1" from US and Canada phone numbers.
...
adacare.lib.PHONE_MAX_DIGITS_AU = 10;       // Exact number of digits in an Australian caller ID
adacare.lib.PHONE_MAX_DIGITS_CA = 10;       // Exact number of digits in a Canadian caller ID (same as U.S.)
adacare.lib.PHONE_MAX_DIGITS_GB = 11;       // Variable number of digits in a UK caller ID!
adacare.lib.PHONE_MAX_DIGITS_GI = 8;        // Exact number of digits in a Gibraltar caller ID
adacare.lib.PHONE_MAX_DIGITS_MT = 8;        // Exact number of digits in a Malta caller ID
adacare.lib.PHONE_MAX_DIGITS_US = 10;       // Exact number of digits in a U.S. caller ID
adacare.lib.PHONE_MAX_DIGITS_ANY = 12;      // Absolute max for any country, to keep the database from blowing up for bad numbers
...
adacare.lib.formatAge = function (birthdate) {
    ...
    var basedate, monthsDiff, yearsDiff;
    var ageString;

    basedate = new Date();
    monthsDiff = adacare.lib.calcAgeMonths(birthdate, basedate);

    if (monthsDiff <= MAX_MONTHS) {
        ageString = (monthsDiff === 1 ? monthsDiff + ' month old' : monthsDiff + ' months old');
    }
    else {
        yearsDiff = adacare.lib.calcAgeYears(birthdate, basedate);
...
/////////////////////////////////////////////////////////////////////////////////////////////////////
// Given two dates, calculate the age difference in years or months.

adacare.lib.calcAgeYears = function (birthdate, basedate) {
    'use strict';
...
adacare.lib.calcAgeMonths = function (birthdate, basedate) {
    'use strict';
    ...
    monthsDiff = (baseY * 12 + baseM) - (birthY * 12 + birthM);

    // Adjust the months if the birthdate hasn't yet come this year. For example, if the birthdate
    // is July 1, subtract a month if the basedate month and day is earlier than the birth date
    // month and day.
        ...
        if ($callerIDField.length > 0 && $callerIDField.val().length === 0) {
            ...
            switch (countryAbbr) {
                case adacare.lib.COUNTRY_ABBR_AU:
                    if (telephoneDigits.length === adacare.lib.PHONE_MAX_DIGITS_AU) {
                        $callerIDField.val(telephoneStr);
                    }
                    break;

                case adacare.lib.COUNTRY_ABBR_CA:
                    if (telephoneDigits.length === adacare.lib.PHONE_MAX_DIGITS_CA) {
                        $callerIDField.val(telephoneStr);
                    }
                    break;
                ...
                case adacare.lib.COUNTRY_ABBR_GI:
                    if (telephoneDigits.length === adacare.lib.PHONE_MAX_DIGITS_GI) {
                        $callerIDField.val(telephoneStr);
                    }
                    break;

                case adacare.lib.COUNTRY_ABBR_MT:
                    if (telephoneDigits.length === adacare.lib.PHONE_MAX_DIGITS_MT) {
                        $callerIDField.val(telephoneStr);
                    }
                    break;
                ...
                default:
                    if (telephoneDigits.length === adacare.lib.PHONE_MAX_DIGITS_US) {
                        $callerIDField.val(telephoneStr);
                    }
                    break;
            }
...
adacare.lib.formatPhoneAU = function (origPhone) {
    ...
    if (cleanedPhone.length > 0) {
        ...
        cleanedPhone = cleanedPhone.substring(0, adacare.lib.PHONE_MAX_DIGITS_AU); // Truncate any excess digits
...
adacare.lib.formatPhoneGB = function (origPhone) {
    ...
    if (cleanedPhone.length > 0) {
        ...
        cleanedPhone = cleanedPhone.substring(0, adacare.lib.PHONE_MAX_DIGITS_GB); // Truncate any excess digits
...
adacare.lib.formatPhoneGI = function (origPhone) {
    ...
    if (cleanedPhone.length > 0) {
        ...
        if (cleanedPhone.length === 11 && cleanedPhone.substring(0, 2) === COUNTRY_CODE_SPAIN) {
            ...
            cleanedPhone = cleanedPhone.substring(0, adacare.lib.PHONE_MAX_DIGITS_GI); // Truncate any excess digits
            formattedPhone = cleanedPhone;
        }
    }
...
adacare.lib.formatPhoneMT = function (origPhone) {
    ...
    if (cleanedPhone.length > 0) {
        ...
        cleanedPhone = adacare.lib.stringCleanDigits(cleanedPhone);
        cleanedPhone = cleanedPhone.substring(0, adacare.lib.PHONE_MAX_DIGITS_MT); // Truncate any excess digits
...
adacare.lib.formatPhoneUS = function (origPhone) {
    ...
    if (cleanedPhone.length > 0) {
        ...
        // Get rid of any leading "1".

        if (cleanedPhone.length > 0 && cleanedPhone.substring(0, 1) === '1') {

            cleanedPhone = cleanedPhone.substring(1);
        }

        cleanedPhone = cleanedPhone.substring(0, adacare.lib.PHONE_MAX_DIGITS_US); // Truncate any excess digits
        ...
        // > 10 is no longer allowed, fixed by cleaning/truncation above.
...
/////////////////////////////////////////////////////////////////////////////////////////////////////
// Limit a text input or textarea element to the given number of characters, and displays
// a count of characters remaining. It should be invoked for every onChange and onKeyUp
// event for the element.
//
// When using .Net controls, implment as follows in caller's aspx page:
//
// <asp:TextBox ID="MyTextBox" runat="server" onKeyUp="javascript:adacare.lib.countAndLimitChars(this, 140, sharedCharCount, 'CharCountDisplay');"
//      onChange="javascript:adacare.lib.countAndLimitChars(this, 140, 'CharCountDisplay');"></asp:TextBox>
//
//      <div id="CharCountDisplay">Nice character count will appear here</div>
//
//      mySharedCharCount: 0; -- JavaScript variable for number of characters, or null if not needed

adacare.lib.countAndLimitChars = function (textElem, maxChars, displayElemId, callbackMethod) {
    'use strict';

    var $textElem, $displayElem;
    var charsUsed, charsRemaining;

    $textElem = $(textElem);
    $displayElem = $('#' + displayElemId);

    charsUsed = $textElem.val().length;

    if (charsUsed > maxChars) {

        $textElem.focus();
        $textElem.val($textElem.val().substr(0, maxChars));
    }

    charsUsed = $textElem.val().length;
    charsRemaining = maxChars - charsUsed;

    if (callbackMethod) { callbackMethod(charsUsed); }
    $displayElem.text(charsRemaining + (charsRemaining === 1 ? " character remaining" : " characters remaining"));
};
