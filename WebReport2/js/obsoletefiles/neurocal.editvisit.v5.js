...
neuro.cal.clientOrStaffDetail = {
    ...
    telephonyID: 'ClientStaffDetailTelephonyID',
...
neuro.cal.editvisit.LinkToClient = function (elemID, updatePanelID, clientID, newView) {
    ...
    //var $dialogDiv = $('#' + neuro.cal.clientOrStaffDetail.divID);
    var client;
...
neuro.cal.editvisit.LinkToStaff = function (elemID, updatePanelID, staffID, newView) {
    ...
    staff = neuro.cal.staffList[staffID];
    ...
    //$('#' + neuro.cal.selectedStaffIDHiddenID).val(String(staffID));
...
neuro.cal.editvisit.OpenDetails = function (title, updatePanelID, clientstaffID, fullname, clientstaffHiddenID, newView, elemID) {
    ...
    //var CSS_LEFT_ARROW = 'cal_detail_popup_left_arrow';              // CSS class of the popup's left arrow

    //var bodyElem = document.getElementById(neuro.cal.bodyID);
    var $elem = $('#' + elemID);
    ...
    $('#' + neuro.cal.clientOrStaffDetail.telephonyID).text('');
    ...
    neuro.cal.initMapButton(neuro.cal.clientOrStaffDetail.mapButtID, 'No Map', 'Wait...', '');
...
neuro.cal.fetchStaffDetailWS = function (staffID) {
    ...
    neuro.cal.officeID, staffID, neuro.cal.begDate,
    neuro.cal.editvisit.WSSuccess,
    neuro.cal.editvisit.WSFailed,
...
neuro.cal.editvisit.Edit = function (e, vElem) {
    ...
    //var junk1 = $('#' + neuro.cal.editDialogVisitIDID).val(); // DEBUG
    v = neuro.cal.visitList[visitID];

    mdy = adacare.lib.formatDate(v.begDateTime);
    //    hh = v.begDateTime.getHours();
    //    mm = v.begDateTime.getMinutes();
    //    hhmm = hh.toString() + ':' + adacare.lib.numPadWithZeros(mm, 2);

    client = neuro.cal.clientList[v.clientID];
    staff = neuro.cal.staffList[v.staffID];
    ...
    $('#' + neuro.cal.editDialogDatePickerID).val(mdy);
    $('#' + neuro.cal.editDialogDatePickerHiddenID).val(mdy);
    ...
    $('#' + neuro.cal.editDialogNotesID).val('');
    ...
    // TODO: Need to define our own button, rather the=an leting the dialog() method
    // define them. This will give us more control over placement and appearance.

    $('#' + neuro.cal.editDialogID).dialog({
        title: 'Edit Visit',
        buttons: {
            'Update': function () { neuro.cal.editvisit.Button(this, visitID, 'update'); },
            'Cancel': function () { neuro.cal.editvisit.Button(this, visitID, 'cancel'); },
            'Delete': function () { neuro.cal.editvisit.Button(this, visitID, 'delete'); }
        },
    ...
    neuro.cal.editvisit.InitStaffForEdit(v.staffID);
...
neuro.cal.editvisit.EraseClientForEdit = function () {
    ...
    //neuro.cal.initMapButton(neuro.cal.editDialogClientMapButtID, 'No Map', '');
};
...
neuro.cal.editvisit.EraseStaffForEdit = function () {
    ...
    //neuro.cal.initMapButton(neuro.cal.editDialogStaffMapButtID, 'No Map', '');
};

neuro.cal.editvisit.InitStaffForEdit = function (staffID) {

    neuro.cal.fetchStaffForEditWS(staffID); // Loads fields and map button asynchronously
};
...
neuro.cal.editvisit.EraseVisitJoinForEdit = function () {
    ...
    $('#' + neuro.cal.editDialogVisitTypeID).unbind();
    $('#' + neuro.cal.editDialogVisitTypeID).change(neuro.cal.editvisit.ChangeVisitTypeSelect);
    ...
    // Fire an event when the visit date is changed.

    $('#' + neuro.cal.editDialogDatePickerHiddenID).unbind();
    $('#' + neuro.cal.editDialogDatePickerHiddenID).change(neuro.cal.editvisit.ChangeDatePicker);

    // Set the arrival/departure TimePickers to fire an event when changed.

    $('#' + neuro.cal.editDialogActualArrivalTimeID).unbind();
    $('#' + neuro.cal.editDialogActualArrivalTimeID).change(neuro.cal.editvisit.ChangeArrivalDepartureSelect);

    $('#' + neuro.cal.editDialogActualDepartureTimeID).unbind();
    $('#' + neuro.cal.editDialogActualDepartureTimeID).change(neuro.cal.editvisit.ChangeArrivalDepartureSelect);

    // Clear the arrival/departure times, and the billing/payroll hours.

    neuro.cal.editvisit.setArrivalDepartureTimes(null, null);
    neuro.cal.editvisit.displayBillingPayrollDefaultMinutes();
    //adacare.vsSpecialRatesEditor.SetValues(neuro.cal.editDialogSpecialRatesEditorSurroundID, VSEValues);
    adacare.vsSpecialRatesEditor.ResetValues(neuro.cal.editDialogSpecialRatesEditorSurroundID);

    $('#' + neuro.cal.editDialogIsConfirmedID).attr('checked', false);
    $('#' + neuro.cal.editDialogIsBillableToClientID).attr('checked', false);
    $('#' + neuro.cal.editDialogIncludeInPayrollID).attr('checked', false);
...
neuro.cal.editvisit.EraseMapUrlsForEdit = function () {
    ...
    neuro.cal.initMapButton(neuro.cal.editDialogClientMapButtID, 'No Map', 'Wait...', '');
    neuro.cal.initMapButton(neuro.cal.editDialogStaffMapButtID, 'No Map', 'Wait...', '');
    neuro.cal.initMapButton(neuro.cal.editDialogOfficeToClientMapButtID, 'No Map', 'Wait...', '');
    neuro.cal.initMapButton(neuro.cal.editDialogStaffToClientMapButtID, 'No Map', 'Wait...', '');
};
...
/////////////////////////////////////////////////////////////////////////////////////////////////////
// Init a map button (used in the edit dialog).

neuro.cal.initMapButton = function (buttonID, buttonEnabledText, buttonDisabledText, mapUrl) {
...
neuro.cal.editvisit.getArrivalDepartureTimes = function (timeFieldID) {
    ...
    if (timeString !== null && timeString !== '') {
        timeVal = adacare.lib.stringToTime(timeString);
...
neuro.cal.editvisit.setArrivalDepartureTimes = function (arrivalDateTime, departureDateTime) {
    ...
    if (arrivalDateTime !== null) {
        $('#' + neuro.cal.editDialogActualArrivalTimeID).val(adacare.lib.FormatTimeStr(arrivalDateTime));
    }
    else {
        $('#' + neuro.cal.editDialogActualArrivalTimeID).val(adacare.lib.FormatTimeMinutesStr(0));
    }

    if (departureDateTime !== null) {
        $('#' + neuro.cal.editDialogActualDepartureTimeID).val(adacare.lib.FormatTimeStr(departureDateTime));
    }
    else {
        $('#' + neuro.cal.editDialogActualDepartureTimeID).val(adacare.lib.FormatTimeMinutesStr(0));
    }
};
...
neuro.cal.editvisit.displayArrivalDepartureTimes = function () {
...
/////////////////////////////////////////////////////////////////////////////////////////////////////
// Display the default values for billing and payroll minutes.

neuro.cal.editvisit.displayBillingPayrollDefaultMinutes = function () {
    ...
    $arrivalTime = $('#' + neuro.cal.editDialogActualArrivalTimeID);
    arrivalTimeParseable = adacare.lib.FormatParseableTimeStr(adacare.lib.stringToTime($arrivalTime.val()));

    $departureTime = $('#' + neuro.cal.editDialogActualDepartureTimeID);
    departureTimeParseable = adacare.lib.FormatParseableTimeStr(adacare.lib.stringToTime($departureTime.val()));

    // The parseable time is added to the visit date to get an accurate date/time.

    arrivalDateTime = new Date($datePicker.val() + ' ' + arrivalTimeParseable);
    departureDateTime = new Date($datePicker.val() + ' ' + departureTimeParseable);
    ...
    minutesDiff = Math.floor((departureDateTime.getTime() - arrivalDateTime.getTime()) / neuro.cal.msPerMinute);
    neuro.cal.editvisit.displayBillingMinutes(minutesDiff);
    neuro.cal.editvisit.displayPayrollMinutes(minutesDiff);
};
...
neuro.cal.editvisit.ChangeVisitFinalStatusSelect = function (eventObject) {
    ...
    var arrivalDateTime, departureDateTime, str;
    ...
    if (visitFinalStatusOrig == neuro.cal.VISITFINALSTATUS_PENDING && visitFinalStatusNew != neuro.cal.VISITFINALSTATUS_PENDING) {
        str = $('#' + neuro.cal.editDialogDatePickerID).val() + ' ' + $('#' + neuro.cal.editDialogTimePickerID).val();
        arrivalDateTime = new Date(str);
        departureDateTime = new Date(arrivalDateTime.getTime() + (v.baseMinutes() + v.extraMinutes) * neuro.cal.msPerMinute);
    ...
    // If the caregiver was a no-show, then we turn off the billable checkbox.
    ...
    neuro.cal.editvisit.displayArrivalDepartureTimes();
    neuro.cal.editvisit.displayBillingPayrollDefaultMinutes();
};
...
neuro.cal.editvisit.ChangeStaffSelect = function (eventObject) {
    ...
    var elemID = this.id;
    var v, visitID, clientID;
    var staffID = Number($('#' + elemID).val());

    neuro.cal.editvisit.EraseStaffForEdit();
    neuro.cal.editvisit.EraseMapUrlsForEditStaff();
    neuro.cal.editvisit.InitStaffForEdit(staffID);
...
neuro.cal.editvisit.Button = function (thisEditDialog, visitID, action) {
    ...
    switch (action) {
        ...
        // User clicked "cancel" -- just close the edit dialog.                                                                                                                                                                                                                                                                                                                                                                                                                                                                            
...
neuro.cal.editvisit.HasScheduleChanges = function (v) {
    ...
    newDateTimeStr = $('#' + neuro.cal.editDialogDatePickerID).val() +
        ' ' + $('#' + neuro.cal.editDialogTimePickerID).val();
    newVisitDateTime = new Date(newDateTimeStr);
    ...
    for (i = 0; i < CalUpdateInfoList.length; i++) {
        ...
        switch (CalUpdateInfo.Action) {
            ...
            case neuro.cal.editvisit.ACTION_ADD:
                ...
                v = neuro.cal.visitList[visitID];
                v.groupNum = neuro.cal.mapStaffOrClientIDToGroupNum(v);
            ...
            case neuro.cal.editvisit.ACTION_DELETE:
                ...
                for (visitID in neuro.cal.visitList) {
                    ...
                    if (((v.scheduleID > 0 && v.scheduleID == scheduleIDBefore) || (v.outcomeID > 0 && v.outcomeID == outcomeIDBefore)) &&
                        ...
                        //var junkvlist0 = neuro.cal.visitList; // DEBUG -- LOOKS LIKE SPLICE DOESN'T WORK ON ASS. ARRAYS?
                        // var junkvlist0len = neuro.cal.visitList.length;
                        neuro.cal.deleteVisit(visitID);
                        //neuro.cal.visitList.splice(vIndex, 1); // This is wrong -- applies to arrays, not objects
                        delete neuro.cal.visitList[visitID]; // Delete the object from the list
                        //var junkvlist1 = neuro.cal.visitList; // DEBUG
                        //var junkvlist1len = neuro.cal.visitList.length;
                        break;
                    }
                    vIndex++;
                }
                ...
                for (visitID in neuro.cal.visitList) {
                    ...
                    if (((v.scheduleID > 0 && v.scheduleID == scheduleIDBefore) || (v.outcomeID > 0 && v.outcomeID == outcomeIDBefore)) &&
                        ...
                        if (CalUpdateInfo.CalVisitJoin.hasProblems != v.hasProblems) {
                            v.hasProblems = CalUpdateInfo.CalVisitJoin.hasProblems;
                            //neuro.cal.editvisit.redrawVisit(v, v.slot); // May want to use just neuro.cal.addWarningClass() instead
...
neuro.cal.editvisit.redrawVisit = function (v, slotOld) {
    ...
    endDateTime = new Date(v.begDateTime.getTime() + (v.baseMinutes() + v.extraMinutes) * neuro.cal.msPerMinute);
    ...
    if ((slotNew >= 0) && (v.groupNum >= 0) && (neuro.cal.withinCalDateSpan(v.begDateTime) || neuro.cal.withinCalDateSpan(endDateTime))) {
        ...
        neuro.cal.placeVisit(v, false);
        if (slotOld != v.slot) {
            neuro.cal.sortVisitsInSlot(slotOld);
        }
    }
...
neuro.cal.editvisit.updateAfterEdit = function (visitID, updateAllFuture) {
    ...
    newIsConfirmed = $('#' + neuro.cal.editDialogIsConfirmedID).attr('checked');
    newIsBillableToClient = $('#' + neuro.cal.editDialogIsBillableToClientID).attr('checked');
    newIncludeInPayroll = $('#' + neuro.cal.editDialogIncludeInPayrollID).attr('checked');

    //newBillingMinutes = Number($('#' + neuro.cal.editDialogBillingMinutesID).val());
    //newBillingMinutes = (isNaN(newBillingMinutes) ? 0 : newBillingMinutes);
    ...
    //newPayrollMinutes = Number($('#' + neuro.cal.editDialogPayrollMinutesID).val());
    //newPayrollMinutes = (isNaN(newPayrollMinutes) ? 0 : newPayrollMinutes);
    ...
    newNotes = $('#' + neuro.cal.editDialogNotesID).val();
...
neuro.cal.editvisit.updateAfterDrag = function (v, vElem, updateAllFuture) {
    ...
    var vElemPos, vElemOffset, slot0Offset;
    var slotNew, visitDate, visitTime;
    ...
    switch (neuro.cal.calType) {
        ...
        case neuro.cal.CALTYPE_STACKED:
            ...
            visitTime = new Date(v.begDateTime.getTime() - visitDate.getTime());
            newVisitDateTime = new Date(neuro.cal.begDate.getTime() + neuro.cal.mapSlotToRowCol(slotNew).col * neuro.cal.msPerDay + visitTime.getTime());
...
neuro.cal.editvisit.deleteVisitJoinWS = function (visitID, updateAllFuture) {
    ...
    var v, origDateUTC, newDateTimeStr, newDateTime, newDateTimeUTC, newDateUTC;
    var begDateCalUTC, endDateCalUTC;
    var newStaffID, newVisitTypeID, newExtraMinutes, newNotes;
    var calVisitJoin = new AdaCareWeb.WebServices.CalVisitJoin();
    ...
    newDateTimeStr = $('#' + neuro.cal.editDialogDatePickerID).val() +
        ' ' + $('#' + neuro.cal.editDialogTimePickerID).val();
    newDateTime = new Date(newDateTimeStr);
    newDateTimeUTC = adacare.lib.convertToUTC(newDateTime);
    newDateUTC = adacare.lib.convertToUTC(neuro.cal.dateTrunc(newDateTime));
    ...
    //calVisitJoin.scheduleTime = newDateTimeUTC;
    //calVisitJoin.extraMinutes = newExtraMinutes;
    //calVisitJoin.notes = newNotes;
    ...
    switch (methodName) {
        ...
        case 'FetchVisitJoin':
            ...
            if (returnedValue !== null) {
                ...
                if (returnedValue.visitID != -1) {
                    notes = adacare.lib.stringNullToEmpty(calVisitJoin.notes);
                }
                ...
                $('#' + neuro.cal.editDialogIsConfirmedID).attr('checked', calVisitJoin.isConfirmed);
                $('#' + neuro.cal.editDialogIsBillableToClientID).attr('checked', calVisitJoin.isBillableToClient);
                $('#' + neuro.cal.editDialogIncludeInPayrollID).attr('checked', calVisitJoin.includeInPayroll);

                neuro.cal.editvisit.setArrivalDepartureTimes(calVisitJoin.actualArrivalDateTime, calVisitJoin.actualDepartureDateTime);
                neuro.cal.editvisit.displayArrivalDepartureTimes();
            ...
            $('#' + neuro.cal.editDialogNotesID).val(notes);
        ...
        case 'FetchClient':
            ...
            if (returnedValue !== null) {
                ...
                neuro.cal.initMapButton(elemID, 'Map', 'No Map', calClient.mapUrl);
            }
        ...
        case 'FetchStaff':
            ...
            if (returnedValue !== null) {
                ...
                elemID = neuro.cal.clientOrStaffDetail.mapButtID;
                neuro.cal.initMapButton(elemID, 'Map', 'No Map', calStaff.mapUrl);
            }
            if (emailAddress.length > 0) { emailAddressUrl = 'mailto:' + emailAddress; }
            $('#' + neuro.cal.clientOrStaffDetail.addressID).text(address);
            $('#' + neuro.cal.clientOrStaffDetail.cityID).text(cityState);
        ...
        case 'FetchMapUrls':
            ...
            if (returnedValue !== null) {
                ...
                neuro.cal.initMapButton(neuro.cal.editDialogClientMapButtID, 'Map', 'No Map', calMap.MapUrlClient);
                neuro.cal.initMapButton(neuro.cal.editDialogStaffMapButtID, 'Map', 'No Map', calMap.MapUrlStaff);
                neuro.cal.initMapButton(neuro.cal.editDialogOfficeToClientMapButtID, 'Map Route from Office', 'No Map', calMap.MapUrlOfficeToClient);
                neuro.cal.initMapButton(neuro.cal.editDialogStaffToClientMapButtID, 'Map Route to Client', 'No Map', calMap.MapUrlStaffToClient);
            }
            break;
...
neuro.cal.fetchClientForEditWSSuccess = function (calClient) {
    ...
    if (calClient !== null) {
        if (calClient.ID != -1) {
            ...
            contactInfo = adacare.lib.stringTrim(calClient.ContactInfo);
            //if (calClient.mapUrl != '') {
            //    elemID = neuro.cal.editDialogClientMapButtID;
            //    neuro.cal.initMapButton(elemID, 'View Map', calClient.mapUrl);
            //}
        }
    }
    if (emailAddress.length > 0) { emailAddressUrl = 'mailto:' + emailAddress; }
...
neuro.cal.fetchClientForEditWSFailed = function (error, context) {
    ...
    if (neuro.cal.HandleAuthenticationError(error)) {
        ...
        neuro.cal.WSFailedGenericMessage(error, context, 'Failed to retrieve client information!');
    }
};
...
neuro.cal.fetchStaffForEditWS = function (staffID) {

    AdaCareWeb.WebServices.CalServices.FetchStaff(neuro.cal.officeID, staffID, neuro.cal.begDate, neuro.cal.fetchStaffForEditWSSuccess, neuro.cal.fetchStaffForEditWSFailed, 'neuro.cal.fetchStaffForEditWS');
};
...
neuro.cal.fetchStaffForEditWSSuccess = function (calStaff) {
    ...
    if (calStaff !== null) {
        if (calStaff.ID != -1) {
            ...
            //if (calStaff.mapUrl != '') {
            //    elemID = neuro.cal.editDialogStaffMapButtID;
            //    neuro.cal.initMapButton(elemID, 'View Map', calStaff.mapUrl);
            //}
        }
    }
    if (emailAddress.length > 0) { emailAddressUrl = 'mailto:' + emailAddress; }
...
neuro.cal.fetchStaffForEditWSFailed = function (error, context) {
    ...
    if (neuro.cal.HandleAuthenticationError(error)) {
        ...
        neuro.cal.WSFailedGenericMessage(error, context, 'Failed to retrieve staff information!');
    }
};
...
neuro.cal.InitVisitJoinForEditWSFailed = function (error, context) {
    ...
    if (neuro.cal.HandleAuthenticationError(error)) {
        ...
        neuro.cal.WSFailedGenericMessage(error, context, 'Failed to retrieve visit information!');
    }
};
...
neuro.cal.editvisit.WSFailed = function (error, context, methodName) {
    ...
    neuro.cal.WSFailedGenericMessage(error, context, methodName + ': Web service failed,');
};
...
neuro.cal.editvisit.UpdateVisitJoinWSFailed = function (error, context) {
    ...
    if (neuro.cal.HandleAuthenticationError(error)) {
        ...
        neuro.cal.WSFailedGenericMessage(error, context, 'Failed to update visit information!');
    }
};
...
neuro.cal.editvisit.DeleteVisitJoinWSFailed = function (error, context) {
    ...
    if (neuro.cal.HandleAuthenticationError(error)) {
        ...
        neuro.cal.WSFailedGenericMessage(error, context, 'Failed to delete visit information!');
    }
};
...
neuro.cal.WSFailedGenericMessage = function (error, context, messsage) {
