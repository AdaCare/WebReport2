...
neuro.cal.addStaff = function (ID, displayRow, fullname) {
    ...
    neuro.cal.staffList[ID] = s; // Consider changing this to "String(ID)". Ditto for addClient().
};
...
neuro.cal.getStaff = function (ID) {
    ...
    for (thisID in neuro.cal.staffList) {
        ...
        //alert('neuro.cal.getStaff, thisID=' + thisID + ', ' + (s ? 's.ID=' + s.ID : 's = undef') + ', match ID==s.id = ' + (ID == s.ID) + ', match givenID==s.id = ' + (givenID == s.ID));
