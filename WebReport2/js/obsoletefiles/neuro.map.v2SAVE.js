...
// Sandy Gettings
// Revised 6/10/2010
// This version is written to MapQuest v6.0
//
...
neuro.map.openMap = function (mapLat, mapLng) {
    ...
    if (mapLat !== null && mapLat !== undefined && mapLng !== null && mapLng !== undefined) {
        ...
        neuro.map.myMap = new MQA.TileMap(mapDivElem, neuro.map.ZOOM_DEFAULT, myLatLng, neuro.map.MAPTYPE_DEFAULT);
    }
    else {
        neuro.map.myMap = new MQA.TileMap(mapDivElem, neuro.map.ZOOM_DEFAULT); //, myLatLng,neuro.map.MAPTYPE_DEFAULT);
    }
...
neuro.map.initMapControls = function () {
    ...
    //'georssdeserializer',
        function () {
            ...
            neuro.map.myMap.addControl(
                new MQA.OverviewMapControl(options),
                new MQA.MapCornerPlacement(MQA.MapCorner.BOTTOM_LEFT)
            );

            // Broken in the MapQuest code - retest soon!
            /***/
            neuro.map.myMap.addControl(
...
neuro.map.drawRoute = function () {
    ...
    //var myRouteResults;
    //var myBoundingBox;
    //var myTrekRoutesColl;
    //var myGC, myLatLng;
    //var routeMinutes, routeMiles, trekText = '';
    var routeHtml, routeElem;
    //var addressItemFrom, addressItemDest;
    var location, locationList = new Array();
    var addressItem, addressCount, a;
    ...
    if (addressCount >= 2) {
        ...
        if (!anyMissing) {
            ...
            //    { street: '104 Ocean Blvd', city: 'Satellite Beach', state: 'FL', postalCode: '32937' },
            //    { street: '441 3rd Ave', city: 'Indialantic', state: 'FL', postalCode: '32903' }
            //    ]);
            //});
            ...
            MQA.withModule('directions', function () {
            neuro.map.myMap.addRoute(locationList,
            { routeOptions: { draggable: true, unit: 'm' },
            ribbonOptions: { draggable: true, draggablepoi: true }
            },
            function (data) {
            ...
            ***/
            ...
            MQA.withModule('directions', function () {
                neuro.map.myMap.addRoute(locationList,
            { routeOptions: { routeType: 'fastest' }, ribbonOptions: { draggable: true, draggablepoi: true} },
            function (data) {
                neuro.map.drawRouteNarrative(data, 0);
                neuro.map.bestFitMap();
            }
...
/***
///////////////////////////////////////////////////////////////////////////////////////////////////
// Return the text for the maneuvers for one leg of the route.

neuro.map.TrekText = function(myTrekRoutesColl) {

var addressItem;
var trekRoutesCount, trekRoute, trekMinutes, trekMiles, tr;
var myManeuverColl, routeManeuvers, m;
var maneuverDist, maneuverText;
var qualityText, qualityHtml, trekText = '';

trekRoutesCount = myTrekRoutesColl.getSize();
for (tr = 0; tr < trekRoutesCount; tr++) {

addressItem = neuro.map.addressList[tr + 1];
qualityText = addressItem.geoqualityText;
...
trekRoute = myTrekRoutesColl.get(tr);
trekMinutes = Math.round(trekRoute.getTime() / 60);
trekMiles = Math.round(trekRoute.getDistance() * 10) / 10;
trekText += '<tr><td>&nbsp;</td><td colspan="2"><div class="map_trek_head">' +
'To ' + addressItem.title + ', ' +
addressItem.street + ' (' +
neuro.map.pluralText(trekMiles, 'mile') + ', ' + neuro.map.pluralText(trekMinutes, 'minute') + ')' +
qualityHtml +
'</div></td></tr>';
myManeuverColl = trekRoute.getManeuvers();

routeManeuvers = myManeuverColl.getSize();
for (m = 0; m < routeManeuvers; m++) {
maneuverDist = Math.round(myManeuverColl.get(m).getDistance() * 10) / 10;
maneuverText = '<tr>' +
'<td class="map_maneuver_rownum">' + (m + 1) + '.' + '</td>' +
'<td>' + myManeuverColl.get(m).getNarrative() + '</td>' +
'<td class="map_maneuver_distance">' + neuro.map.pluralText(maneuverDist, 'mile') + '</td>' +
'</tr>';
trekText += maneuverText;
}
}
return trekText;
};
