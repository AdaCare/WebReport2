...
//neuro.cal.editDialogUseSpecialRatesID = '';
//neuro.cal.editDialogBillingUseThisRateID = '';
//neuro.cal.editDialogBillingFlatRateID = '';
//neuro.cal.editDialogBillingHourlyRateID = '';
//neuro.cal.editDialogBillingTravelRateID = '';
//neuro.cal.editDialogPayrollUseThisRateID = '';
//neuro.cal.editDialogPayrollFlatRateID = '';
//neuro.cal.editDialogPayrollHourlyRateID = '';
//neuro.cal.editDialogPayrollTravelRateID = '';

neuro.cal.addVisitAccordionID = '';
neuro.cal.addVisitAccordionIndex = 0;
neuro.cal.addVisitClientPickerID = '';
neuro.cal.addVisitStartDatePickerID = '';
neuro.cal.addVisitStartDatePickerHiddenID = '';
neuro.cal.addVisitThruDateOpenEndedID = '';
neuro.cal.addVisitThruDatePickerID = '';
neuro.cal.addVisitThruDatePickerHiddenID = '';
neuro.cal.addVisitTimePickerID = '';
neuro.cal.addVisitSchedulingPrefsID = '';
...
//neuro.cal.masterpage_outerClassDefault = 'masterpage_outer';
//neuro.cal.masterpage_outerClassFullWidth = 'masterpage_outer_fullwidth';
...
neuro.cal.visitClassDirty = 'cal_visitcell_dirty';
...
neuro.cal.visitClassInfo = 'cal_visitcell_info';
//neuro.cal.visitClassResizeGrip = 'cal_visitcell_resize_grip';
...
neuro.cal.msPerDay = 60 * 60 * 24 * 1000;                   // Precalculate milliseconds per day.
neuro.cal.msPerMinute = 60 * 1000;                          // Precalculate milliseconds per minute.
...
neuro.cal.initStacked = function (
    ...
    // Calculate column width by comparing two adjacent slots.
    // Note: The code below works, just not used for the sake of consistency with initOverlay().

    //slotCol0 = $('#' + neuro.cal.slotID(0));
    //slotCol1 = $('#' + neuro.cal.slotID(neuro.cal.rows));
    //neuro.cal.colWidth = slotCol1.offset().left - slotCol0.offset().left;
    ...
    neuro.cal.endDateTime = new Date(dt.getTime() + neuro.cal.msPerDay * neuro.cal.colsPerGroup - 1);
    ...
    $('#' + innersurroundID).find('*').each(function (i) { neuro.cal.makeUnselectable(this); });

    // Next, sort all of the visits in each slot. This function has the extra duty of
    // placing a space into empty slots. Otherwise, the table cells won't show their
    // borders.

    // TODO: Isn't this premature? No visits have been added as yet...
    //for (i = 0; i < neuro.cal.rows * neuro.cal.colsPerGroup * groups; i++) {
    //    neuro.cal.sortVisitsInSlot(i);
    //}
...
neuro.cal.initCommon = function () {
    ...
    neuro.cal.visitList = new Array();                          // List of visits
    neuro.cal.groupList = new Array();                          // List of column groups
    neuro.cal.staffList = new Array();                          // List of staff
    neuro.cal.clientList = new Array();                         // List of clients
    neuro.cal.visitTypeList = new Array();                      // List of visit types
    neuro.cal.adjustCalendarWidth();

    // Update the master page's width to full width

    //    if (!$('.' + neuro.cal.masterpage_outerClassDefault).hasClass(neuro.cal.masterpage_outerClassFullWidth)) {
    //        $('.' + neuro.cal.masterpage_outerClassDefault).addClass(neuro.cal.masterpage_outerClassFullWidth);
    //    }
    ...
    if (isInside) {
        ...
        switch (neuro.cal.calType) {
            ...
            case neuro.cal.CALTYPE_STACKED:
                ...
                visitDate = new Date(neuro.cal.begDate.getTime() + neuro.cal.mapSlotToRowCol(slotNew).col * neuro.cal.msPerDay);
                visitDate = neuro.cal.dateTrunc(visitDate);
        ...
        // TODO: Need to add staff to DropDownList, because it's usually an empty list.

        if (clientID) {
            $('#' + neuro.cal.addVisitClientPickerID).val(clientID);
            neuro.cal.fetchClientForAddVisitWS(clientID);
        }
        ...
        $('#' + neuro.cal.addVisitStartDatePickerID).val(adacare.lib.formatDate(visitDate));
        $('#' + neuro.cal.addVisitStartDatePickerHiddenID).val(adacare.lib.formatDate(visitDate));
        //$('#' + neuro.cal.addVisitThruDateOpenEndedID).attr('checked', 'checked');
        $('#' + neuro.cal.addVisitThruDateOpenEndedID).prop('checked', true);
        //$('#' + neuro.cal.addVisitThruDatePickerID).val(adacare.lib.formatDate(visitDate));
        //$('#' + neuro.cal.addVisitThruDatePickerHiddenID).val(adacare.lib.formatDate(visitDate));
        if (visitTime) {
...
neuro.cal.pleaseWaitPopup = function (action) {
    ...
    switch (action) {
        ...
        case 'open':
            //neuro.cal.pleaseWaitTimeoutID = setTimeout('adacare.lib.pleaseWaitPopup("open");', DELAY_MS);
            adacare.lib.pleaseWaitPopup('open');
            //neuro.cal.pleaseWaitTimeoutID = setTimeout('$("#pleaseWaitDiv").block();', DELAY_MS);
            break;

        case 'close':
            //clearTimeout(neuro.cal.pleaseWaitTimeoutID);
            //neuro.cal.pleaseWaitTimeoutID = -1;
            adacare.lib.pleaseWaitPopup('close');
            //$("#pleaseWaitDiv").unblock();
            break;
    }
};
...
//editDialogStaffMapButtID,
    ...
    neuro.cal.editDialogStaffIDID = editDialogStaffIDID;                            // Staff ID field
    ...
    neuro.cal.editDialogNotesID = editDialogNotesID;                                // Notes field
    ...
    //    neuro.cal.editDialogUseSpecialRatesID = editDialogUseSpecialRatesID;
    //    neuro.cal.editDialogBillingUseThisRateID = editDialogBillingUseThisRateID;
    //    neuro.cal.editDialogBillingFlatRateID = editDialogBillingFlatRateID;
    //    neuro.cal.editDialogBillingHourlyRateID = editDialogBillingHourlyRateID;
    //    neuro.cal.editDialogBillingTravelRateID = editDialogBillingTravelRateID;
    //    neuro.cal.editDialogPayrollUseThisRateID = editDialogPayrollUseThisRateID;
    //    neuro.cal.editDialogPayrollFlatRateID = editDialogPayrollFlatRateID;
    //    neuro.cal.editDialogPayrollHourlyRateID = editDialogPayrollHourlyRateID;
    //    neuro.cal.editDialogPayrollTravelRateID = editDialogPayrollTravelRateID;
...
neuro.cal.initAddVisit = function (
        addVisitAccordionID,
        addVisitAccordionIndex,
        addVisitClientPickerID,
        addVisitStartDatePickerID,
        addVisitStartDatePickerHiddenID,
        addVisitThruDateOpenEndedID,
        addVisitThruDatePickerID,           // TODO: Remove?
        addVisitThruDatePickerHiddenID,     // TODO: Remove?
        addVisitTimePickerID,
        addVisitSchedulingPrefsID) {
...
neuro.cal.VisitType = function (ID, shortName, minutesLength, descrip) {
...
neuro.cal.Visit = function (visitID, outcomeID, scheduleID, visitDate, groupNum, clientID, staffID, begDateTime, visitTypeID, extraMinutes, visitFinalStatus, hasProblems, freqOnce, isAvailable) {
    ...
    this.notes = '';
    //this.slot = neuro.cal.mapDateTimeToSlot(groupNum, begDateTime);
...
neuro.cal.Visit.prototype.endDateTime = function () { return new Date(this.begDateTime.getTime() + (this.baseMinutes() + this.extraMinutes) * neuro.cal.msPerMinute); };
...
neuro.cal.addClient = function (ID, displayRow, fullname) {
    ...
    // Count the number of elements in the array. Since the array
    // is sparse, we can't use the "length" property.
    // TODO: Think about the caller providing the row number?

    //    for (var i in neuro.cal.clientList) {
    //        rowCount++;
    //    }

    c = new neuro.cal.Client(ID, displayRow, fullname);
    neuro.cal.clientList[ID] = c;
};
...
neuro.cal.addStaff = function (ID, displayRow, fullname) {
    ...
    //    for (var i in neuro.cal.staffList) {
    //        rowCount++;
    //    }

    s = new neuro.cal.Staff(ID, displayRow, fullname);
    neuro.cal.staffList[ID] = s;
};

///////////////////////////////////////////////////////////////////////////////////////////////////
// Add a visit type to the calendar.

neuro.cal.addVisitType = function (ID, shortName, minutesLength, descrip) {

    var vt;

    vt = new neuro.cal.VisitType(ID, shortName, minutesLength, descrip);
...
//
// There are two versions of this funstion, one used by local JavaScript code, and another that is
...
neuro.cal.addVisitFromServer = function (visitID, outcomeID, scheduleID, visitDateStr, groupNum, clientID, staffID, begDateTimeStr, visitTypeID, extraMinutes, visitFinalStatus, hasProblems, freqOnce, isAvailable) {

    var hasProblemsBool, freqOnceBool, isAvailableBool;

    // Server code passes in 0 or 1 from the HTML stream, so we convert to boolean.

    hasProblemsBool = Boolean(hasProblems !== 0);
    freqOnceBool = Boolean(freqOnce !== 0);
    isAvailableBool = Boolean(isAvailable !== 0);

    neuro.cal.addVisit(visitID, outcomeID, scheduleID, visitDateStr, groupNum, clientID, staffID, begDateTimeStr, visitTypeID, extraMinutes, visitFinalStatus, hasProblemsBool, freqOnceBool, isAvailableBool);
};

// Use this version for local JavaScript calls.

neuro.cal.addVisit = function (visitID, outcomeID, scheduleID, visitDateStr, groupNum, clientID, staffID, begDateTimeStr, visitTypeID, extraMinutes, visitFinalStatus, hasProblems, freqOnce, isAvailable) {
    ...
    // Original code:
    //Boolean(hasProblems != 0),      // Server code passes in 0 or 1, so we convert to boolean. 
    //Boolean(freqOnce != 0),         // Server code passes in 0 or 1, so we convert to boolean.
    //Boolean(isAvailable != 0));     // Server code passes in 0 or 1, so we convert to boolean.

    neuro.cal.visitList[visitID] = v;
    //neuro.cal.constructVisitElem(v, visitID, visitID);
    //neuro.cal.placeVisit(v);
};
...
neuro.cal.displayAllVisits = function () {
    ...
    //adacare.lib.profileStart('displayAllVisits');
    ...
    //adacare.lib.profileStop('displayAllVisits');
    //adacare.lib.profileReport();
};
...
neuro.cal.constructVisitElem = function (v, visitID, parentVisitID) {
    ...
    //adacare.lib.profileStart('constructVisitElem');
    var $newElem;
    var vElemID;
    var vElemRepeaterID;
    var vElemHeaderID;
    ...
    elemFullname = (neuro.cal.viewWho == neuro.cal.VIEWWHO_CLIENT) ? neuro.cal.staffList[v.staffID].fullname : neuro.cal.clientList[v.clientID].fullname;
    ...
    //neuro.cal.addWarningClass(v, $newElem);

    // Add these attribute as coded below. Incorporating them in the template doesn't
    // seem to work in Safari.

    $newElem[0].name = neuro.cal.VISIT_ELEMNAME;
    $newElem[0].visitID = parentVisitID;

    // Make the visit element (and its children) unselectable.

    //$newElem.find('*').andSelf().each(function() { neuro.cal.makeUnselectable(this); });
};
...
///////////////////////////////////////////////////////////////////////////////////////////////////
// Add or remove the "warning" CSS class to the visit as needed.

neuro.cal.addWarningClass = function (v, $vElem) {

    //adacare.lib.profileStart('addWarningClass');
    var needWarning, isNoShow, isUnassigned;
    var $vElemRepeater;
    var $tempElem;

    $vElemRepeater = $('#' + neuro.cal.elemRepeaterID(v.visitID));
    //needWarning = (v.staffID == neuro.cal.STAFF_UNASSIGNED_ID) || v.hasProblems;
    ...
    //    // Unassigned staff get a special style in their name (the header DIV)

    //    $tempElem = $vElem.find('.' + neuro.cal.visitClassHeader);

    //    if ($tempElem.length > 0) {
    //        $tempElem.removeClass(neuro.cal.visitClassHeaderUnassigned);

    //        if (v.staffID == neuro.cal.STAFF_UNASSIGNED_ID) {
    //            $tempElem.addClass(neuro.cal.visitClassHeaderUnassigned);
    //        }
    //    }
    ...
    // Add the main class to the visit element
    ...
    // These are add-on classes, to append an icon to the visit.

    if (!v.freqOnce) {
        $vElemRepeater.addClass(neuro.cal.visitClassRepeater);
        //$vElem.prepend('<div class="' + neuro.cal.visitClassRepeater + '">&nbsp;</div>');
        //$vElem.find('.' + neuro.cal.visitClassRepeater).height($vElem.height()).width($vElem.width());
    }
    ...
    if (isNoShow) {
        //$vElem.addClass(neuro.cal.visitClassNoShow);
    ...
    //adacare.lib.profileStop('addWarningClass');
};

///////////////////////////////////////////////////////////////////////////////////////////////////
// Return the DOM element ID of the given visit, and of its children.

neuro.cal.elemID = function (visitID) { return 'visit_' + visitID; };
neuro.cal.elemRepeaterID = function (visitID) { return 'visit_' + visitID + '_repeater'; };
neuro.cal.elemInnerID = function (visitID) { return 'visit_' + visitID + '_inner'; };
neuro.cal.elemHeaderID = function (visitID) { return 'visit_' + visitID + '_header'; };
neuro.cal.elemTimeID = function (visitID) { return 'visit_' + visitID + '_time'; };
neuro.cal.elemMinutesID = function (visitID) { return 'visit_' + visitID + '_minutes'; };
neuro.cal.elemInfoID = function (visitID) { return 'visit_' + visitID + '_info'; };
neuro.cal.elemResizeGripID = function (visitID) { return 'visit_' + visitID + '_resizeGrip'; };
neuro.cal.visitOflowID = function (visitID) { return visitID.toString() + 'oflow'; };
neuro.cal.elemOflowID = function (visitID) { return 'visit_' + neuro.cal.visitOflowID(visitID); };
...
neuro.cal.placeVisit = function (v, isInitialLoad) {
    ...
    //adacare.lib.profileStart('placeVisit');
    switch (neuro.cal.calType) {
        case neuro.cal.CALTYPE_OVERLAY:
    ...
    //adacare.lib.profileStop('placeVisit');
};
...
neuro.cal.placeOverlayVisit = function (v, isInitialLoad) {
    ...
    if (slotRowColBot.col == slotRowColTop.col) {
        ...
        //if (v.dirty) {
        //    $vElem.addClass(neuro.cal.visitClassDirty);
        //}
        ...
        if ((slotRowColTop.col + 1) < neuro.cal.colsPerGroup) {
            ...
            neuro.cal.addWarningClass(v, $vElemOflow);
...
neuro.cal.placeStackedVisit = function (v, isInitialLoad) {
    ...
    //adacare.lib.profileStart('placeStackedVisit');
    ...
    elemWidth = neuro.cal.visitWidth;
    //slotLeftPx = $('#' + slotID).position().left;
    ...
    //if (v.dirty) {
    //    $('#' + vElemID).addClass(neuro.cal.visitClassDirty);
    //}
    //neuro.cal.enableWidgetsForOneVisit(v.visitID);
    neuro.cal.sortVisitsInSlot(v.slot, isInitialLoad);
    //adacare.lib.profileStop('placeStackedVisit');
};
...
neuro.cal.sortVisitsInSlot = function (slot, isInitialLoad) {
    ...
    //adacare.lib.profileStart('sortVisitsInSlot');
    var slotID;
    var $kids, kidCount;
    ...
    if (neuro.cal.calType == neuro.cal.CALTYPE_STACKED) {
        ...
        $kids = $('#' + slotID).children('[name="visit"]'); // Get list of visit DOM elements in the slot.
    ...
    //adacare.lib.profileStop('sortVisitsInSlot');
};
...
neuro.cal.deleteVisit = function (visitID) {
    ...
    neuro.cal.sortVisitsInSlot(v.slot);
    v.dirty = true;
    v.hidden = true;
    v.deleted = true;
};
...
neuro.cal.withinCalDateSpan = function (dt) {
    ...
    //    calBeg = neuro.cal.begDateTime;
    //    calMs = neuro.cal.slotsPerGroup * neuro.cal.msPerSlot;
    //    calEnd = new Date(calBeg.getTime() + calMs);

    //    result = (dt >= calBeg && dt < calEnd);
    //    return result;
...
neuro.cal.mapVisitToSlotByParms = function (clientID, staffID, begDateTime, groupNum) {
    ...
    switch (neuro.cal.calType) {
        case neuro.cal.CALTYPE_OVERLAY:
            ...
            //col = (vDate.getTime() - neuro.cal.begDate.getTime()) / neuro.cal.msPerDay + groupNum * neuro.cal.colsPerGroup;
            col = days + groupNum * neuro.cal.colsPerGroup;
            slot = row + col * neuro.cal.rows;
            break;

        case neuro.cal.CALTYPE_STACKED:
            // mapVisitToRow() may return -1 if the client or staff is not on the calendar body.
            days = neuro.cal.dateDaysBetween(vDate, neuro.cal.begDate);
            row = neuro.cal.mapVisitToRowByParms(clientID, staffID);
            //col = (vDate.getTime() - neuro.cal.begDate.getTime()) / neuro.cal.msPerDay;
            col = days;
            // DEBUG
            //            var junk1 = vDate.getTime();
            //            var junk2 = neuro.cal.begDate.getTime();
            //            var junkmsperday = neuro.cal.msPerDay;
            //            var junkdiff = junk1 - junk2;
            //            var junkdiv = junkdiff / junkmsperday;
            // DEBUG
            if (row >= 0) {
                slot = row + col * neuro.cal.rows;
            }
...
neuro.cal.mapSlotToDateTime = function (slot) {
    ...
    var slotWithinGroup;
    var slotMs, dateTimeMs;
    var dateTime;

    switch (neuro.cal.calType) {
        case neuro.cal.CALTYPE_OVERLAY:
            // Multiple time slots in each column

            slotWithinGroup = slot % neuro.cal.slotsPerGroup;
            slotMs = neuro.cal.msPerSlot * slotWithinGroup;
            dateTimeMs = neuro.cal.begDateTime.getTime() + slotMs;
            dateTime = new Date(dateTimeMs);
            break;

        case neuro.cal.CALTYPE_STACKED:
        default:
            // Each column represents and entire day
...
neuro.cal.mapSlotToGroupNum = function (slot) {
    ...
    switch (neuro.cal.calType) {
        ...
        case neuro.cal.CALTYPE_STACKED:
            // Groups are in rows
            //groupNum = slot % neuro.cal.rows; // Orig
            groupNum = Math.floor(slot / (neuro.cal.rows * neuro.cal.colsPerGroup));
            break;
    }
...
neuro.cal.mapVisitToRowByParms = function (clientID, staffID) {
    ...
    //    if (rowNum < 0) {
    //        throw new Error("neuro.cal.mapVisitToRow: Staff ID #" + staffID.toString() + " is not in staffList!");
    //    }
...
neuro.cal.mapCoordToSlot = function (top, left) {
    ...
    switch (neuro.cal.calType) {
        case neuro.cal.CALTYPE_OVERLAY:
            // Fixed-height rows
            row = Math.floor(top / neuro.cal.rowHeight);
            col = Math.floor(left / neuro.cal.colWidth);
            break;
...
///////////////////////////////////////////////////////////////////////////////////////////////////
// Map a staffID to a groupNum.

neuro.cal.mapStaffIDToGroupNumOBSOLETE = function (staffID) {
    ...
    // Stacked calendars have only one group. 
...
neuro.cal.mapStaffOrClientIDToGroupNum = function (v) {
...
neuro.cal.timeStr = function (dt) {
    ...
    var hour, hourStr, min, minStr, ampmStr;
    var result;
...
neuro.cal.visitMinutesStr = function (v) {
    ...
    //    lenHours = Math.floor(lenMins / 60);
    //    lenMins = lenMins % 60;

    //    if (lenHours > 0) lenStr = lenHours.toString() + 'h';
...
neuro.cal.dateDaysBetween = function (d1, d2) {
    ...
    //var junk1 = d1.getTimezoneOffset(); // DEBUG
    ms1 = d1.getTime() - d1.getTimezoneOffset() * neuro.cal.msPerMinute;
    ms2 = d2.getTime() - d2.getTimezoneOffset() * neuro.cal.msPerMinute;
    days = (ms1 - ms2) / neuro.cal.msPerDay;
...
/////////////////////////////////////////////////////////////////////////////////////////////////////
// Web services for database ScheduledVisits table.

// Update the visit.

// DEPRECATED
//neuro.cal.updateVisitWS = function(v) {

//    var calVisit = new AdaCareWeb.WebServices.CalVisit();
//    var begDateTimeUTC;
...
//    //year = v.begDateTime.getFullYear();
//    //month = v.begDateTime.getMonth();
//    //day = v.begDateTime.getDate();
//    //hours = v.begDateTime.getHours();
//    //minutes = v.begDateTime.getMinutes();
//    //begDateTimeUTC = new Date(Date.UTC(year, month, day, hours, minutes, 0, 0));
//    begDateTimeUTC = adacare.lib.convertToUTC(v.begDateTime);

//    calVisit.visitID = v.visitID;
//    calVisit.IsManual = true;
//    calVisit.clientID = v.clientID;
//    calVisit.staffID = v.staffID;
//    calVisit.begDateTime = begDateTimeUTC;
//    calVisit.visitTypeID = v.visitTypeID;
//    calVisit.extraMinutes = v.extraMinutes;
//    calVisit.notes = v.notes;

//    AdaCareWeb.WebServices.CalServices.UpdateScheduledVisit(calVisit, neuro.cal.updateVisitWSSuccess, neuro.cal.sharedVisitWSFailed, 'neuro.cal.updateVisitWS');
//};

//neuro.cal.updateVisitWSSuccess = function(results) {
...
//neuro.cal.deleteVisitWS = function(visitID) {

//    AdaCareWeb.WebServices.CalServices.DeleteScheduledVisit(visitID, neuro.cal.deleteVisitWSSuccess, neuro.cal.sharedVisitWSFailed, 'neuro.cal.deleteVisitWS');
//};

//neuro.cal.deleteVisitWSSuccess = function(results) {
