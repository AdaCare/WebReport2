...
//
// neuro.wsgrid.v1.3                Sandy Gettings              Revised 04/10/2012
//
...
neuro.wsgrid.init = function (scrollingDivID, maxWeeks, minPerSlot, scrollToMinute, tabDivID, repeatWeeksID) {

    // Constants

    neuro.wsgrid.CSS_SELECTED = 'ui-selected';
    neuro.wsgrid.MINUTES_IN_DAY = 24 * 60;
    neuro.wsgrid.maxDayNum = -1;
    neuro.wsgrid.scrollCurrent = 0;

    neuro.wsgrid.slotsPerHour = 60 / minPerSlot;
    neuro.wsgrid.scrollingDivID = scrollingDivID;
    neuro.wsgrid.maxWeeks = maxWeeks;
    neuro.wsgrid.minPerSlot = minPerSlot;
    neuro.wsgrid.scrollToMinute = scrollToMinute;
    neuro.wsgrid.tabDivID = tabDivID;
    neuro.wsgrid.repeatWeeksID = repeatWeeksID;

    neuro.wsgrid.handleRepeatWeeksChange();
    $('#' + tabDivID).unbind('tabsshow').bind('tabsshow', neuro.wsgrid.handleTabShow);
    $('#' + scrollingDivID).unbind('scroll').scroll(neuro.wsgrid.handleScroll);
    $('#' + repeatWeeksID).unbind('change').change(neuro.wsgrid.handleRepeatWeeksChange);
};
...
neuro.wsgrid.initDay = function (dayNum, dayGridID, dayParmID, summaryID, availableOptID, reasonID, allDayOnButtID, allDayOffButtID, copyID, copyDayNumFrom) {
    ...
    neuro.wsgrid.maxDayNum = (dayNum > neuro.wsgrid.maxDayNum ? dayNum : neuro.wsgrid.maxDayNum); // Remember how many days we're working with
    ...
    if ($dayGridKids.length === 0) {
        ...
        // JQUERY BUG FIX! DELETE WHEN FIXED IN JQUERY! JQUERY BUG FIX! DELETE WHEN FIXED IN JQUERY! JQUERY BUG FIX! DELETE WHEN FIXED IN JQUERY!
        // Should be fixed in jquery-ui-1.8.17 or jquery-ui-1.9.
        //
        // The problem: Control+click in the grid acts just like a regular click. Thus, can't unselect individual grid cells.

        $dayGrid.selectable().on('mousedown', '*', function (e) {
            if (e.ctrlKey)
                e.metaKey = e.ctrlKey;
        });

        // END BUG FIX END BUG FIX END BUG FIX END BUG FIX END BUG FIX END BUG FIX END BUG FIX END BUG FIX END BUG FIX END BUG FIX
    }
...
neuro.wsgrid.redrawAllGrids = function () {
    ...
    for (dayNum = 0; dayNum <= neuro.wsgrid.maxDayNum; dayNum++) {
...
neuro.wsgrid.setScroll = function () {
    ...
    //$('.wsgrid_scrollingdiv').scrollTop(junkoh * scrollIndex);
    neuro.wsgrid.scrollCurrent = junkoh * scrollIndex;
    neuro.wsgrid.syncScroll();
};

/////////////////////////////////////////////////////////////////////////////////////////////////////
// Sync the scroll on all tabs to the current scroll value.

neuro.wsgrid.syncScroll = function () {

    $('.wsgrid_scrollingdiv').scrollTop(neuro.wsgrid.scrollCurrent);
};

/////////////////////////////////////////////////////////////////////////////////////////////////////
// Handle a scrolling event. We just remember the current scroll, so that the scrolls for all tabs
// can be synced.

neuro.wsgrid.handleScroll = function (e) {

    var divID;
    var divScroll;

    divID = e.currentTarget.id;
    divScroll = $('#' + divID).scrollTop();
    neuro.wsgrid.scrollCurrent = divScroll;
};
...
neuro.wsgrid.handleTabShow = function (e, ui) {
...
neuro.wsgrid.handleRepeatWeeksChange = function () {
    ...
    tabDivID = neuro.wsgrid.tabDivID;
    $tabDiv = $('#' + tabDivID);
    $tabDiv.tabs('select', 0);

    // Disable any weeks after the desired number.

    maxWeeks = neuro.wsgrid.maxWeeks;
    repeatWeeks = Number($('#' + neuro.wsgrid.repeatWeeksID).val());
