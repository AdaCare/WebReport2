...
neuro.cal.fetchBillingAndPayrollRatesWS = function () {
    ...
    if (!VSEValues.billingUseThisRate || !VSEValues.payrollUseThisRate) {
        ...
        effectiveDate = adacare.lib.parseDateLocalized(neuro.cal.countryAbbr, effectiveDateStr);
...
neuro.cal.editvisit.Edit = function (e, vElem) {
    ...
    //dateStrLocalized = adacare.lib.formatDate(v.begDateTime);
    dateStrLocalized = adacare.lib.formatDateLocalized(neuro.cal.countryAbbr, v.begDateTime);
...
neuro.cal.editvisit.EraseVisitJoinForEdit = function () {
    ...
    //neuro.cal.editvisit.displayBillingPayrollDefaultMinutesOBSOLETE();
...
neuro.cal.editvisit.getArrivalDepartureTimes = function (timeFieldID) {
    ...
    if (timeString !== null && timeString !== '') {
        ...
        timeFieldDateTime = adacare.lib.parseDateTimeLocalized(neuro.cal.countryAbbr, dateFieldVal, timeParseable);
    }
...
neuro.cal.editvisit.displayBillingPayrollDefaultMinutesOBSOLETE = function () {
...
neuro.cal.recalcArrivalDepartureTimes = function (v) {
    ...
    arrivalDateTime = adacare.lib.parseDateTimeLocalized(neuro.cal.countryAbbr, dateStr, timeStr);
    departureDateTime = new Date(arrivalDateTime.getTime() + visitMinutes * neuro.cal.msPerMinute);
    neuro.cal.editvisit.setArrivalDepartureTimes(arrivalDateTime, departureDateTime);
    neuro.cal.updateBillingAndPayrollHoursFromEdit();
};
...
neuro.cal.calcActualVisitMinutesFromEdit = function () {
    ...
    //arrivalDateTime = new Date($datePicker.val() + ' ' + arrivalTimeParseable);
    //departureDateTime = new Date($datePicker.val() + ' ' + departureTimeParseable);
    arrivalDateTime = adacare.lib.parseDateTimeLocalized(neuro.cal.countryAbbr, $datePicker.val(), arrivalTimeParseable);
    departureDateTime = adacare.lib.parseDateTimeLocalized(neuro.cal.countryAbbr, $datePicker.val(), departureTimeParseable);
    ...
    actualMinutes = Math.floor((departureDateTime.getTime() - arrivalDateTime.getTime()) / neuro.cal.msPerMinute);
...
neuro.cal.editvisit.HasScheduleChanges = function (v) {
    ...
    dateStr = $('#' + neuro.cal.editDialogDatePickerID).val();
    timeStr = $('#' + neuro.cal.editDialogTimePickerID).val();
    newVisitDateTime = adacare.lib.parseDateTimeLocalized(neuro.cal.countryAbbr, dateStr, timeStr);
...
neuro.cal.editvisit.redrawSchedule = function (CalUpdateInfoList) {
    ...
    for (i = 0; i < CalUpdateInfoList.length; i++) {
        ...
        //visitDate = adacare.lib.convertFromUTC(CalUpdateInfo.VisitDate);
        //visitDate = CalUpdateInfo.VisitDate;
        //visitDate = new Date(CalUpdateInfo.VisitDateStr);
        visitDate = CalUpdateInfo.VisitDate;
        var junkedit = new Date(CalUpdateInfo.VisitDateStr); // DEBUG
        editDateValue = new Date(CalUpdateInfo.VisitDateStr).valueOf(); // We must compare date *values*, not date *objects*!
        //editDate = adacare.lib.parseDateLocalized(neuro.cal.countryAbbr, CalUpdateInfo.VisitDateStr);
        //editDateValue = editDate.valueOf(); // We must compare date *values*, not date *objects*!
...
neuro.cal.editvisit.updateAfterEdit = function (visitID, updateAllFuture) {
    ...
    //newDateTimeStr = $('#' + neuro.cal.editDialogDatePickerID).val() + ' ' + $('#' + neuro.cal.editDialogTimePickerID).val();
    //newVisitDateTime = new Date(newDateTimeStr);
    dateStr = $('#' + neuro.cal.editDialogDatePickerID).val();
    timeStr = $('#' + neuro.cal.editDialogTimePickerID).val();
    newVisitDateTime = adacare.lib.parseDateTimeLocalized(neuro.cal.countryAbbr, dateStr, timeStr);
...
neuro.cal.editvisit.deleteVisitJoinWS = function (visitID, updateAllFuture) {
    ...
    var dateStr, timeStr;
    var v, origDateUTC, newDateTime, newDateTimeUTC, newDateUTC;
    var begDateCalUTC, endDateCalUTC;
    ...
    dateStr = $('#' + neuro.cal.editDialogDatePickerID).val();
    timeStr = $('#' + neuro.cal.editDialogTimePickerID).val();
    newDateTime = adacare.lib.parseDateTimeLocalized(neuro.cal.countryAbbr, dateStr, timeStr);
