...
//
// adacare.lib.v2.2                Sandy Gettings              Revised 5/18/11
//
...
// 1. The ID of the text field element where the date appears.
// 2. The ID of a hidden field element with a copy of the date.
// 3. The ID of the "previous" button image element.
// 4. The ID of the "next" button image element.
// 5. An object with options as follows:
//
...
//                                          will stop at the uper's date and not cross over.
//
...
adacare.lib.initDatePickerV2 = function (elemID, elemHiddenID, prevButtID, nextButtID, options) {

    var ONE_DAY_MS = 86400000;
    var SIX_DAY_MS = 86400000 * 6;
    ...
    function SetDatePicker(origDate, newDate) {
        ...
        newDateStr = adacare.lib.formatDate(newDate);
        $elem.val(newDateStr);
    ...
    function BindToHandler(thisDate, bindToType, $bindToElem, $bindToElemHidden) {
        ...
        if (bindToType !== undefined && $bindToElem !== undefined && $bindToElemHidden !== undefined) {
            ...
            if (bindToType === BIND_TO_TYPE_LOWERSTOP && thisDate < bindToDate) {
                ...
                $bindToElem.val(adacare.lib.formatDate(bindToDate));
    ...
    function PrevNextHandler($elem, $elemHidden, incrementDays) {
        ...
        var origDate, newDate, newDateStr;
        //var finalDate;
        var newDayOfWeek;
        var needPostBack = false;

        origDate = new Date($elem.val());
