// Copyright 2009 - 2012 by Neurosoftware, LLC.
//
// neuro.cal.editvisit.v5.34.js
// Sandy Gettings
//
...
neuro.cal.editvisit.Edit = function (e, vElem) {
    ...
    neuro.cal.editDialogStaffSelectMenuUpdateRecent([]);
    ...
    $('#' + neuro.cal.editDialogID).dialog({
        ...
        open: function () {
            ...
            // neuro.cal.initEditDialogStaffSelectMenu();
    ...
    //neuro.cal.editvisit.InitClientForEdit(v.clientID, v.visitDate);
    ...
    //neuro.cal.editVisit.LoadAllData(v); // Changed 11/15/14 SG
...
/////////////////////////////////////////////////////////////////////////////////////////////////////
// Fetch all data for the Edit dialog.

//neuro.cal.editVisit.LoadAllData = function (v) {

//neuro.cal.fetchClientForEditWS(v.clientID, v.visitDate); // Loads fields and map button asynchronously
//neuro.cal.fetchStaffForEditWS(v.staffID, v.visitDate); // Loads fields and map button asynchronously
//neuro.cal.editvisit.InitVisitJoinForEdit(v);
//neuro.cal.fetchMapUrlsWS(v.clientID, v.staffID);
...
// 11/15/14 SG Removed
//neuro.cal.editvisit.InitClientForEdit = function (clientID, visitDate) {
//
//    neuro.cal.fetchClientForEditWS(clientID, visitDate); // Loads fields and map button asynchronously
//};
...
neuro.cal.editvisit.showOrHideArrivalDepartureTimes = function () {
    ...
    var $billingMinutes, $billingTravelDistance, $payrollMinutes, $payrollTravelDistance;
...
neuro.cal.editvisit.ChangeStaffSelect = function (selectedValue, selectedText) {
    ...
    //$('#' + neuro.cal.editDialogSelectedStaffIDNEW).val('' + staffID);
    //var junk0 = $('#' + neuro.cal.editDialogSelectedStaffIDNEW); // DEBUG
    //var junk1 = $('#' + neuro.cal.editDialogSelectedStaffIDNEW).val(); // DEBUG
    neuro.cal.editDialogSelectedStaffIDNEW = staffID;

    neuro.cal.editvisit.EraseStaffForEdit();
    neuro.cal.editvisit.EraseMapUrlsForEditStaff();
    //neuro.cal.editvisit.InitStaffForEdit(staffID, v.visitDate); // 11/15/14 SG Replaced with line below
    neuro.cal.fetchStaffForEditWS(staffID, v.visitDate); // Loads fields and map button asynchronously

    //neuro.cal.editvisit.InitMapUrlsForEditStaff(clientID, staffID); // 11/15/14 SG Replaced with line below
    neuro.cal.fetchMapUrlsWS(clientID, staffID);
    neuro.cal.fetchBillingAndPayrollRatesWS();
};
...
neuro.cal.editvisit.updateAfterEdit = function (visitID, updateAllFuture) {
    ...
    var newBillingMinutes, newBillingTravelDistance, newPayrollMinutes, newPayrollTravelDistance;
    ...
    neuro.cal.editvisit.updateVisitJoinWS(v, updateAllFuture, false, newVisitDateTime, newStaffID, newContractID, newVisitTypeID, newExtraMinutes,
        ...
        newBillingMinutes, newBillingTravelDistance, newPayrollMinutes, newPayrollTravelDistance,
        VSEValues.billingUseThisRate, VSEValues.billingFlatRate, VSEValues.billingHourlyRate, VSEValues.billingTravelDistanceRate,
        VSEValues.payrollUseThisRate, VSEValues.payrollFlatRate, VSEValues.payrollHourlyRate, VSEValues.payrollTravelDistanceRate,
...
neuro.cal.editvisit.updateAfterDrag = function (v, vElem, updateAllFuture) {
    ...
    neuro.cal.editvisit.updateVisitJoinWS(v, updateAllFuture, true,
        newVisitDateTime, newStaffID, 0, newVisitTypeID, newExtraMinutes, '', null, null, null, false, false, false, 0, 0, 0, 0,
...
neuro.cal.editvisit.updateVisitJoinWS = function (v, updateAllFuture, updateScheduleOnly,
    ...
    newVisitFinalStatus, newVisitTagIDList, newActualArrivalDateTime, newActualDepartureDateTime, newIsConfirmed, newIsBillableToClient, newIncludeInPayroll,
    newBillingMinutes, newBillingTravelDistance, newPayrollMinutes, newPayrollTravelDistance,
    newBillingUseThisRate, newBillingFlatRate, newBillingHourlyRate, newBillingTravelDistanceRate,
    newPayrollUseThisRate, newPayrollFlatRate, newPayrollHourlyRate, newPayrollTravelDistanceRate,
    newVisitNotes, newSupervisorNotes) {
...
neuro.cal.editvisit.WSSuccess = function (returnedValue, context, methodName) {
    ...
    switch (methodName) {
        ...
        case 'FetchVisitJoin':
            ...
            neuro.cal.editvisit.displayVisitJoinForEditWS(returnedValue);
...
neuro.cal.editvisit.displayClientForEditWS = function (calClient) {
    ...
    //var calClient = new AdaCareWeb.WebServices.CalClient();
    var address = '', cityState = '', resPhone = '', mobilePhone = '', contactInfo = '', emailAddress = '', emailAddressUrl = '', billingMinutesToString = '';
...
/***
neuro.cal.fetchClientForEditWS = function (clientID, visitDate) {

    AdaCareWeb.WebServices.CalServices.FetchClient(neuro.cal.officeID, clientID, visitDate,
        neuro.cal.fetchClientForEditWSSuccess,
        neuro.cal.fetchClientForEditWSFailed,
        'neuro.cal.fetchClientForEditWS');
};
...
neuro.cal.fetchClientForEditWSSuccess = function (calClient) {
    ...
    neuro.cal.editvisit.displayClientForEditWS(calClient);
};
...
/*** OBSOLETE 2014-12-14 SG
// Fetch a client for adding a new visit.

neuro.cal.fetchClientForAddVisitWS = function (clientID, visitDate) {

    AdaCareWeb.WebServices.CalServices.FetchClient(neuro.cal.officeID, clientID, visitDate,
        neuro.cal.fetchClientForAddVisitWSSuccess,
        neuro.cal.fetchClientForAddVisitWSFailed,
        'neuro.cal.fetchClientForAddVisitWS');
};
...
neuro.cal.fetchClientForAddVisitWSSuccess = function (calClient) {
    ...
    if (activeEndDate != null) {
        ...
        if (visitDate.getTime() > activeEndDate.getTime()) {
            ...
            adacare.DatePickerV3.setDateByExternalCode(neuro.cal.addVisitStartDatePickerID, activeEndDateLocalizedStr.getFullYear(), activeEndDateLocalizedStr.getMonth(), activeEndDateLocalizedStr.getDate()); // Changed for datepickerV3.1
