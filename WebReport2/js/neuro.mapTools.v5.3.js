...
//
// neuro.mapTools.v5.3.js
// Sandy Gettings
//
// This version is written to MapQuest JS v1.3.1 (uses Leaflet)
//
...
// 2018-06-05 SG: Minor improvement/fixes since conversion to new MapQuesp API.
// 2019-02-19 SG: MapQuest only allows "A-Za-z0-9" for markers. Changed to restrict staff initials used for markers to these characters.
...
neuro.mapTools = {
    ...
    DEFAULT_LAT: 38.8890264,
    DEFAULT_LNG: -77.0507608,
    ...
    initMobileMap: function (mapDivID, mapManeuversDivID) {
        'use strict';
    ...
    drawMap: function (mapInfo, routeHeading) {
        ...
        if (mapInfo.addressList.length > 0) {
            ...
            if (mapInfo.addressList[0].latLng.lat && mapInfo.addressList[0].latLng.lng) {

                startLatLng = new neuro.mapTools.LatLng(mapInfo.addressList[0].latLng.lat, mapInfo.addressList[0].latLng.lng);
            }
            else {

                startLatLng = new neuro.mapTools.LatLng(neuro.mapTools.DEFAULT_LAT, DEFAULT_LNG);
            }
        }
        else {
        ...
        neuro.mapTools.openMap(mapInfo, startLatLng.lat, startLatLng.lng);
    ...
    openMap: function (mapInfo, mapLat, mapLng) {
        ...
        if (mapLat && mapLng) {
            ...
            // Display an error -- this should never happen.

            alert('Sorry, but the map latitude/longitude is not valid for some reason. Please report this issue to AdaCare Customer Support.');

            mapInfo.myMap = new L.mapquest.map(
                mapInfo.mapDivID, {
                    center: [neuro.mapTools.DEFAULT_LAT, neuro.mapTools.DEFAULT_LNG],
                    layers: L.mapquest.tileLayer('map'),
                    zoom: neuro.mapTools.ZOOM_DEFAULT
                });
        }
    ...
    clearMap: function (mapInfo) {
        ...
        if (mapInfo.myMap) {
            ...
            // 2019-04-26 SG: Fixed bug when resrting clock-in/out map after error.
            // Sometimes mapInfo.myMap exists, but mapInfo.myMap.off() does not.

            //mapInfo.myMap.off();
            //mapInfo.myMap.remove();

            if (mapInfo.myMap.off) {
    ...
    geocodeAddressListForHERE: function (mapInfo) {
        ...
        var platform, getGeocodeParms, geocodingService, anyNeedGeocoding;
        ...
        if (addressCount >= 2) {
            ...
            if (!anyMissing) {
                ...
                directions.route({
                    ...
                    //routeType: mapInfo.routeTypeOption
    ...
    directionsCallback: function (mapInfo, error, response, routeHeading) {
        ...
        directionsLayer = L.mapquest.directionsLayer({
            ...
            startMarker: {
                draggable: false,
                icon: 'marker', iconOptions: { primaryColor: '#339933', shadow: false }
            },
            endMarker: {
                draggable: false,
                icon: 'marker', iconOptions: { primaryColor: '#990000', shadow: false }
            },
            waypointMarker: {
                draggable: false,
                iconOptions: { shadow: false }
            }
        }).addTo(mapInfo.myMap);
    ...
    addGpsDeviceIconToMap: function (mapInfo, lat, lng, staffNameInitials) {
        ...
        var BAD_CHAR_REPLACEMENT = 'X'; // Must be an alphanumeric!

        var icon, symbolText, c, i;

        // 2019-02-19 SG: In the code below that creates icon, we use the staff's initials.
        // They require the initials to be only alphanumeric characters, A-Z a-z 0-9. Since
        // the code will blow up if the initials contain anything else (like punctuation
        // characters), we have to filter those out.

        symbolText = '';

        for (i = 0; i < staffNameInitials.length; i++) {

            c = String(staffNameInitials.charAt(i));
            ...
            if (c.match("^[A-Za-z0-9]+$")) {
                ...
                symbolText = symbolText + BAD_CHAR_REPLACEMENT;
            }
        }
