...
//
// 2013-10-11
// 2013-12-19 SG: Fix bug in changing visit final status. It was using the old status in comparison.
...
// 2017-07-27 SG: Performance improvements for displaying large numbers of visits.
// 2017-07-31 SG: Changes to support better client/staff detail pop-up.
// 2017-10-17 SG: Format contact info and scheduling prefs in pop-up client/staff detail to include newlines.
// 2018-01-01 SG: Added option to preload the message box with the caller's text.
// 2018-01-08 SG: Bug fix for preloaded message box.
// 2018-01-22 SG: Version change for force browsers to load newest version.
// 2018-04-20 SG: Added support for emergency phone numbers.
// 2018-08-08 SG: Updated "VisitTemplate" to "VisitTemplate".
// 2018-08-24 SG: Added support for ClientPayers in editing visits.
// 2018-10-04 SG: Added support for insurance authorizations.
// 2018-11-19 SG: Fixed problem with leftover insurance authorization dialog.
// 2018-11-27 SG: Fixed "cancel" button click for insurance authorization firing multiple times.
// 2019-06-05 SG: Changed display to show "recorded" hours.
// 2019-06-11 SG: Bug fix for initially displaying scheduled departure time. Also modified setArrivalDepartureTimes() to display null times as blank instead of midnight.
...
neuro.cal.editvisit.InsuranceAuthorizationInfoPanelID = 'InsuranceAuthorizationInfoPanel';
neuro.cal.editvisit.ViewAuthorizationButtID = 'ViewAuthorization';
neuro.cal.InsuranceAuthorizationDisplayToolDialogID = 'InsuranceAuthorizationDisplayToolDialog';
neuro.cal.InsuranceAuthorizationDisplayToolPlanNameFullID = 'InsuranceAuthorizationPlanNameFull';
neuro.cal.InsuranceAuthorizationDisplayToolProblemTextID = 'InsuranceAuthorizationProblemText';
neuro.cal.InsuranceAuthorizationDisplayToolInfoBlockID = 'InsuranceAuthorizationDisplayInfoBlock';

// When we start to edit a visit, save a copy of the original. We'll compare this to edits when
// the user saves the changes back to the server.

neuro.cal.editvisit.calAllVisitDataOriginal = null;

// Remember the CalVisitJoin.ClientPayerInfo list. We need the DefaultContractID whenever the user
// selects a different ClientPayer in the drop-down list.

neuro.cal.clientPayerInfoList = [];
...
neuro.cal.clientOrStaffDetail = {
    ...
    emergencyPhoneID: 'ClientStaffDetailEmergencyPhone',
...
neuro.cal.editvisit.LinkToClient = function (elemID, updatePanelID, clientID, newView) {
    ...
    client = neuro.cal.getClient(clientID);
...
neuro.cal.editvisit.OpenDetails = function (title, updatePanelID, clientstaffID, fullname, clientstaffHiddenID, newView, elemID) {
    ...
    currView = $('#' + neuro.cal.selectedViewModeHiddenID).val();
    ...
    $('#' + neuro.cal.clientOrStaffDetail.emergencyPhoneID).text('');
    ...
    $closeBox.off('click'); // 2018-01-08 SG:Unbind() is deprecated in jQuery 3
    ...
    var $elemParentToAppendTo;

    if (currView === neuro.cal.VIEW_MODE_DAILY_ALL_RESOURCE_VIEW) {

        // Client or staff is on the header of the calendar. We'll append to the div that contains all of the
        // float-left divs that each hold a name hyperlink.

        $elemParentToAppendTo = $elem.parent().parent();
        $dialogSurroundDiv.appendTo($elemParentToAppendTo);
        $dialogSurroundDiv.css({
            'top': $elem.position().top - 6,
            'left': $elem.parent().position().left + $elem.parent().outerWidth() - 6,
            'z-index': 1000
        });
        ...
        // Client or staff is on the left side of the calendar. We'll append to the same table cell where the
        // person's name hyperlink sits.

        $elemParentToAppendTo = $elem.parent();
        $dialogSurroundDiv.appendTo($elemParentToAppendTo);
        $dialogSurroundDiv.css({
            'top': $elem.position().top - 6,
            'left': $elem.parent().outerWidth() - 6,
            'z-index': 1000
        });
    }
...
neuro.cal.fetchBillingAndPayrollRatesWS = function () {
    ...
    var v, visitID, scheduleID, staffID, contractID, visitTemplateID, effectiveDate, effectiveDateStr;
    ...
    if (!VSEValues.billingUseThisRate || !VSEValues.payrollUseThisRate) {
        ...
        visitID = neuro.cal.$editDialogVisitID.val();
        ...
        visitTemplateID = Number($('#' + neuro.cal.editDialogVisitTemplateID).val());
        ...
        AdaCareWeb.WebServices.CalServices.FetchBillingAndPayrollRates(
            scheduleID, staffID, contractID, visitTemplateID, effectiveDate,
            neuro.cal.editvisit.WSSuccess,
            neuro.cal.editvisit.WSFailed,
...
neuro.cal.editvisit.Edit = function (e, vElem) {
    ...
    neuro.cal.$editDialogVisitID.val(visitID); // Remember the visit ID - other functions may need it later
    v = neuro.cal.visitList[visitID];

    dateStrLocalized = adacare.lib.formatDateLocalized(adacare.lib.countryAbbr, v.begDateTime);

    client = neuro.cal.getClient(v.clientID);
    ...
    $('#' + neuro.cal.editDialogVisitTemplateID).val(v.visitTemplateID.toString());
    ...
    $('#' + neuro.cal.editDialogBaseMinutesID).text(neuro.cal.getVisitTemplateMinutesFormatted(v.visitTemplateID));
    ...
    ***/
    ...
    $editDialog = neuro.cal.$editDialog;
...
neuro.cal.editvisit.EraseClientForEdit = function () {
    ...
    $('#' + neuro.cal.editDialogClientEmergencyPhoneID).text('');
...
neuro.cal.editvisit.EraseStaffForEdit = function () {
    ...
    $('#' + neuro.cal.editDialogStaffEmergencyPhoneID).text('');
...
neuro.cal.editvisit.EraseVisitJoinForEdit = function () {
    ...
    // Fire an event when the ClientPayer is changed. Note that the ClientPayer drop-down list is
    // cleared here, and populated when we fetch the client's info.

    $('#' + neuro.cal.editDialogClientPayerID).off('change');
    $('#' + neuro.cal.editDialogClientPayerID).empty();
    $('#' + neuro.cal.editDialogClientPayerID).change(neuro.cal.editvisit.ChangeClientPayerSelect);

    // Fire an event when the contract is changed.

    $('#' + neuro.cal.editDialogContractID).off('change');
    ...
    // Fire an event when the visit template is changed.

    $('#' + neuro.cal.editDialogVisitTemplateID).off('change').change(neuro.cal.editvisit.ChangeVisitTemplateSelect);

    // Fire an event when the visit final status is changed.

    //$('#' + neuro.cal.editDialogVisitFinalStatusID).unbind(); // 2018-01-08 SG:Unbind() is deprecated in jQuery 3
    $('#' + neuro.cal.editDialogVisitFinalStatusID).off('change');
    ...
    //
    // Please note the two bindings on the "chgange" handler. If the DatePick widget is changed, you better
    // retest to make sure the events get fired.
    //$('#' + neuro.cal.editDialogDatePickerID).off().change(neuro.cal.editvisit.ChangeDatePicker); // This seems to work, but prevent DatePicker from popping up
    //$('#' + neuro.cal.editDialogDatePickerID).off('change').change(neuro.cal.editvisit.ChangeDatePicker); // This seems to work, but DatePicker keeps popping back up
    $('#' + neuro.cal.editDialogDatePickerID).off('change', neuro.cal.editvisit.ChangeDatePicker).change(neuro.cal.editvisit.ChangeDatePicker); // This seems to work, but DatePicker keeps popping back up
    //$('#' + neuro.cal.editDialogDatePickerHiddenID).off().change(neuro.cal.editvisit.ChangeDatePicker); // This doesn't

    $('#' + neuro.cal.editDialogTimePickerID).off('change').change(neuro.cal.editvisit.ChangeTimePicker);

    // Fire an event when the extra minutes selection is changed.

    $('#' + neuro.cal.editDialogExtraMinutesID).off('change').change(neuro.cal.editvisit.ChangeExtraMinutesSelect); // 2018-01-08 SG:Unbind() is deprecated in jQuery 3

    // Set the arrival/departure TimePickers to fire an event when changed.

    $('#' + neuro.cal.editDialogRecordedArrivalTimeID).off('change').change(neuro.cal.editvisit.ChangeArrivalDepartureSelect);
    $('#' + neuro.cal.editDialogRecordedDepartureTimeID).off('change').change(neuro.cal.editvisit.ChangeArrivalDepartureSelect);

    // Clear the arrival/departure times, and the billing/payroll hours.

    neuro.cal.editvisit.setArrivalDepartureTimes(null, null, null);
...
neuro.cal.editvisit.initArrivalDepartureTimes = function (v, visitFinalStatus, scheduledDepartureDateTime, recordedArrivalDateTime, recordedDepartureDateTime) {
    'use strict';
    ...
    if (visitFinalStatus === neuro.cal.VISITFINALSTATUS_PENDING) {
        ...
        neuro.cal.editvisit.setArrivalDepartureTimes(scheduledDepartureDateTime, recordedArrivalDateTime, recordedDepartureDateTime);
    }
};

neuro.cal.editvisit.setArrivalDepartureTimes = function (scheduledDepartureDateTime, recordedArrivalDateTime, recordedDepartureDateTime) {
    'use strict';

    if (scheduledDepartureDateTime !== null) {

        $('#' + neuro.cal.editDialogScheduledDepartureTimeID).val(adacare.lib.FormatTimeLocalized(adacare.lib.countryAbbr, scheduledDepartureDateTime));
    }
    else {

        //$('#' + neuro.cal.editDialogScheduledDepartureTimeID).val(adacare.lib.FormatTimeMinutesLocalized(adacare.lib.countryAbbr, 0));
        $('#' + neuro.cal.editDialogScheduledDepartureTimeID).val('');
    }

    if (recordedArrivalDateTime !== null) {

        $('#' + neuro.cal.editDialogRecordedArrivalTimeID).val(adacare.lib.FormatTimeLocalized(adacare.lib.countryAbbr, recordedArrivalDateTime));
    }
    else {

        //$('#' + neuro.cal.editDialogRecordedArrivalTimeID).val(adacare.lib.FormatTimeMinutesLocalized(adacare.lib.countryAbbr, 0));
        $('#' + neuro.cal.editDialogRecordedArrivalTimeID).val('');
    }

    if (recordedDepartureDateTime !== null) {

        $('#' + neuro.cal.editDialogRecordedDepartureTimeID).val(adacare.lib.FormatTimeLocalized(adacare.lib.countryAbbr, recordedDepartureDateTime));
    }
    else {

        //$('#' + neuro.cal.editDialogRecordedDepartureTimeID).val(adacare.lib.FormatTimeMinutesLocalized(adacare.lib.countryAbbr, 0));
        $('#' + neuro.cal.editDialogRecordedDepartureTimeID).val('');
    }
};
...
neuro.cal.editvisit.showOrHideArrivalDepartureTimes = function () {
    ...
    $arrivalTime = $('#' + neuro.cal.editDialogRecordedArrivalTimeID);
    $departureTime = $('#' + neuro.cal.editDialogRecordedDepartureTimeID);
...
//    Use the visit template's estimated minutes, or the minutes defined by the visit template and extra minutes.
// Otherwise:
...
neuro.cal.editvisit.setInitialBillingAndPayrollHours = function (v, isBillableToClient, visitOutcomeBillingMinutes, includeInPayroll, visitOutcomePayrollMinutes) {
    ...
    var visitTemplate, vtID;
    var visitFinalStatus;
    var visitMinutes;
    var billingMinutes, payrollMinutes;

    visitFinalStatus = v.visitFinalStatus;
    vtID = v.visitTemplateID;
    visitTemplate = neuro.cal.visitTemplateList[vtID];

    if (visitFinalStatus === neuro.cal.VISITFINALSTATUS_PENDING) {

        visitMinutes = neuro.cal.calcVisitMinutes(v.visitTemplateID, v.extraMinutes);
        billingMinutes = (visitTemplate.billingUseEstimate ? visitTemplate.billingMinutesEstimate : visitMinutes);
        payrollMinutes = (visitTemplate.payrollUseEstimate ? visitTemplate.payrollMinutesEstimate : visitMinutes);
    }
    else {
...
neuro.cal.updateBillingAndPayrollHoursFromEdit = function () {
    ...
    var visitTemplateID, visitTemplate;
    var recordedMinutes;
    var billingMinutes, payrollMinutes;

    visitTemplateID = Number($('#' + neuro.cal.editDialogVisitTemplateID).val());
    visitTemplate = neuro.cal.visitTemplateList[visitTemplateID];
    recordedMinutes = neuro.cal.calcRecordedVisitMinutesFromEdit();

    billingMinutes = (visitTemplate.billingUseEstimate ? visitTemplate.billingMinutesEstimate : recordedMinutes);
    payrollMinutes = (visitTemplate.payrollUseEstimate ? visitTemplate.payrollMinutesEstimate : recordedMinutes);
...
neuro.cal.recalcArrivalDepartureTimes = function (v) {
    ...
    var scheduledArrivalDateTime, scheduledDepartureDateTime, recordedArrivalDateTime, recordedDepartureDateTime;
    var visitTemplateID, visitMinutes;
    var scheduledDateStr, scheduledTimeStr;

    visitTemplateID = Number($('#' + neuro.cal.editDialogVisitTemplateID).val());
    visitMinutes = neuro.cal.calcVisitMinutes(visitTemplateID, neuro.cal.getExtraMinutesFromEdit());

    scheduledDateStr = $('#' + neuro.cal.editDialogDatePickerID).val();
    scheduledTimeStr = $('#' + neuro.cal.editDialogTimePickerID).val();

    scheduledArrivalDateTime = adacare.lib.parseDateTimeLocalized(adacare.lib.countryAbbr, scheduledDateStr, scheduledTimeStr);
    scheduledDepartureDateTime = new Date(scheduledArrivalDateTime.getTime() + visitMinutes * neuro.cal.MS_PER_MINUTE);

    // In this function, the recorded times are the same as the scheduled times.

    recordedArrivalDateTime = adacare.lib.parseDateTimeLocalized(adacare.lib.countryAbbr, scheduledDateStr, scheduledTimeStr);
    recordedDepartureDateTime = new Date(recordedArrivalDateTime.getTime() + visitMinutes * neuro.cal.MS_PER_MINUTE);
...
/////////////////////////////////////////////////////////////////////////////////////////////////////
// Calculate the minutes for the visit. This function uses the visit template minutes and any extra
// minutes in the visit.

neuro.cal.calcVisitMinutes = function (visitTemplateID, extraMinutes) {
    'use strict';
    ...
    visitTemplate = neuro.cal.visitTemplateList[visitTemplateID];
    visitMinutes = visitTemplate.minutesLength + extraMinutes;
...
/////////////////////////////////////////////////////////////////////////////////////////////////////
// Calculate recordedMinutes visit minutes from the two time fields.

neuro.cal.calcRecordedVisitMinutesFromEdit = function () {
    'use strict';
    ...
    recordedMinutes = Math.floor((departureDateTime.getTime() - arrivalDateTime.getTime()) / neuro.cal.MS_PER_MINUTE);
...
/////////////////////////////////////////////////////////////////////////////////////////////////////
// Handle a change in selection of the ClientPayer in the edit dialog. The ClientPayer ID is selected
// in the DropDownList referenced by "this".

neuro.cal.editvisit.ChangeClientPayerSelect = function (eventObject) {
    'use strict';

    var clientPayerID, defaultContractID;
    var i;

    // Look up the new ClientPayer's default contract and set that as the new contract.

    clientPayerID = Number($('#' + neuro.cal.editDialogClientPayerID).val());

    for (i = 0; i < neuro.cal.clientPayerInfoList.length; i++) {

        if (neuro.cal.clientPayerInfoList[i].ID === clientPayerID) {

            defaultContractID = neuro.cal.clientPayerInfoList[i].DefaultContractID;
            $('#' + neuro.cal.editDialogContractID).val(defaultContractID);
            neuro.cal.displayInsuranceAuthorizationDiv();
            break;
        }
    }
};
...
/////////////////////////////////////////////////////////////////////////////////////////////////////
// Handle a change in selection of the visit template in the edit dialog. The visit template ID is selected 
// in the DropDownList referenced by "this".

neuro.cal.editvisit.ChangeVisitTemplateSelect = function (eventObject) {
    'use strict';
...
neuro.cal.editvisit.ChangeDatePicker = function (eventObject) {
    ...
    // The billing and payroll special rates needs to be reloaded.

    neuro.cal.fetchBillingAndPayrollRatesWS();

    // The messaging pop-up text needs to be reloaded.

    neuro.cal.editvisit.reloadMessagingOpenPopupJavaScriptCode(v.clientID, Number(neuro.cal.editDialogSelectedStaffIDNEW));
};
...
neuro.cal.editvisit.ChangeTimePicker = function (eventObject) {
    ...
    //var dateStr, timeStr, visitDateTime, preloadedMessageTextForClient, preloadedMessageTextForStaff;
    var v, visitID;
...
//
// The user clicked Update, Cancel, or Delete in the edit dialog. Updates and deletes for repeating 
// schedules are a little more complicated, as we ask the user if this is "this visit only" or "all
// future visits."
...
neuro.cal.editvisit.Button = function ($editDialog, visitID, action) {
    ...
    switch (action) {
        ...
        case 'update':
            ...
            isValid = $('form').validate().element('#' + neuro.cal.editDialogRecordedArrivalTimeID)
                && $('form').validate().element('#' + neuro.cal.editDialogRecordedDepartureTimeID);
...
// the user if she wants to update all future visits is moot.

neuro.cal.editvisit.HasScheduleChanges = function (v) {
    'use strict';

    var dateStr, timeStr;
    var newVisitDateTime, newStaffID, newVisitTemplateID, newExtraMinutes;
    var origClientPayerID, newClientPayerID;
    var origContractID, newContractID;
    var hasChanges;
    ...
    origClientPayerID = neuro.cal.editvisit.calAllVisitDataOriginal.CalVisitJoin.ClientPayerID;
    newClientPayerID = Number($('#' + neuro.cal.editDialogClientPayerID).val());

    origContractID = neuro.cal.editvisit.calAllVisitDataOriginal.CalVisitJoin.ContractID;
    newContractID = Number($('#' + neuro.cal.editDialogContractID).val());

    newStaffID = Number(neuro.cal.editDialogSelectedStaffIDNEW);
    newVisitTemplateID = Number($('#' + neuro.cal.editDialogVisitTemplateID).val());
    ...
    hasChanges =
        ...
        || (newClientPayerID !== origClientPayerID)
        || (newContractID !== origContractID)
        || (newVisitTemplateID != v.visitTemplateID)
...
neuro.cal.editvisit.RedrawAfterCancelDrag = function (v, vElem) {
    ...
    if (neuro.cal.calType === neuro.cal.CALTYPE_STACKED) {
        ...
        //$('#' + vElem.id).animate({ left: 0, top: 0 }, "slow"); // 2017-07-28 SG
        $vElem.animate({ left: 0, top: 0 }, "slow");
    }
    neuro.cal.placeVisit(v, $vElem, false);
};
        ...
        switch (CalUpdateInfo.Action) {
            ...
            case neuro.cal.editvisit.ACTION_ADD:
                ...
                neuro.cal.addVisit(
                    ...
                    CalUpdateInfo.CalVisitJoin.VisitTemplateID,
                ...
                $newElem = neuro.cal.constructSingleVisitElem(v, visitID, visitID);
                neuro.cal.$body.append($newElem);
                neuro.cal.placeVisit(v, $newElem, false);
            ...
            case neuro.cal.editvisit.ACTION_UPDATE:
                ...
                for (visitID in neuro.cal.visitList) {
                    ...
                    if (((v.scheduleID > 0 && v.scheduleID === scheduleIDBefore) || (v.outcomeID > 0 && v.outcomeID === outcomeIDBefore)) &&
                        ...
                        v.visitTemplateID = CalUpdateInfo.CalVisitJoin.VisitTemplateID;
...
neuro.cal.editvisit.redrawVisit = function (v, slotOld) {
    ...
    var vElemOflowID, $vElemOflow;
    var vElemOflowTimeID, vElemOflowMinutesID, vElemOflowHeaderID;
    var slotNew;
    ...
    if ((slotNew >= 0) && (v.groupNum >= 0) && (neuro.cal.withinCalDateSpan(v.begDateTime) || neuro.cal.withinCalDateSpan(endDateTime))) {
        ...
        elemFullname = (neuro.cal.viewWho === neuro.cal.VIEWWHO_CLIENT) ? neuro.cal.getStaff(v.staffID).fullname : neuro.cal.getClient(v.clientID).fullname;
        ...
        // Update the overflow element's name and times *after* placeVisit() invoked. This way we can 
        // detect whether the overflow still exists or not. For example, the user may have changed the 
        // time or duration of the visit so that the verflow is no longer needed.

        vElemOflowID = neuro.cal.visitOflowID(v.visitID);
        $vElemOflow = $('#' + neuro.cal.elemOflowID(v.visitID));

        if ($vElemOflow.length > 0) {

            vElemOflowHeaderID = neuro.cal.elemHeaderID(vElemOflowID);
            vElemOflowTimeID = neuro.cal.elemTimeID(vElemOflowID);
            vElemOflowMinutesID = neuro.cal.elemMinutesID(vElemOflowID);

            $('#' + vElemOflowHeaderID).text(elemFullname);
            $('#' + vElemOflowTimeID).text(neuro.cal.visitTimeStr(v));
            $('#' + vElemOflowMinutesID).text(neuro.cal.timeStr(v.endDateTime()));
        }

        // Should the slot be sorted regardless?
...
neuro.cal.editvisit.updateAfterEdit = function (visitID, updateAllFuture) {
    ...
    calVisitJoin = neuro.cal.editvisit.constructCalVisitJoinAfterEdit(visitID);
    neuro.cal.editvisit.updateVisitJoinWS(calVisitJoin, updateAllFuture, false);
};
...
neuro.cal.editvisit.updateAfterDrag = function (v, vElem, updateAllFuture) {
    ...
    var newVisitDate, newVisitDateTime, newStaffID, newVisitTemplateID, newExtraMinutes;
    var vElemPos, vElemOffset, slot0Offset;
    ...
    /***
    calVisitJoin = neuro.cal.editvisit.constructCalVisitJoin(v,
        newVisitDateTime, newStaffID, 0, 0, newVisitTemplateID, newExtraMinutes,
    ...
    ***/
    ...
    calVisitJoin = neuro.cal.editvisit.constructCalVisitJoinAfterDragOrResize(v, newVisitDateTime, newStaffID, newVisitTemplateID, newExtraMinutes);
    neuro.cal.editvisit.updateVisitJoinWS(calVisitJoin, updateAllFuture, true);
};
...
/////////////////////////////////////////////////////////////////////////////////////////////////////
// Construct a CalVisitJoin object. It's used to pass VisitJoin info back to the server's web services
// for several kinds of updates (edits, drags, and so forth).

neuro.cal.editvisit.constructCalVisitJoin = function (v,
    newVisitDateTime, newStaffID, newClientPayerID, newContractID, newVisitTemplateID, newExtraMinutes,
    newVisitFinalStatus, newVisitTagIDList, newRecordedArrivalDateTime, newRecordedDepartureDateTime, newIsConfirmed, newIsBillableToClient, newIncludeInPayroll,
    ...
    calVisitJoin.ClientPayerID = newClientPayerID;
    calVisitJoin.ContractID = newContractID;
    calVisitJoin.VisitTemplateID = newVisitTemplateID;
    ...
    calVisitJoin.RecordedArrivalDateTime = (newRecordedArrivalDateTime !== null ? adacare.lib.convertToUTC(newRecordedArrivalDateTime) : null);
    calVisitJoin.RecordedDepartureDateTime = (newRecordedDepartureDateTime !== null ? adacare.lib.convertToUTC(newRecordedDepartureDateTime) : null);
...
// Same as above, except we also incorporate the user's edit in the pop-up edit dialog.

neuro.cal.editvisit.constructCalVisitJoinAfterEdit = function (visitID) {
    'use strict';

    var v;
    var dateStr, timeStr;
    var newVisitDateTime, newStaffID, newClientPayerID, newContractID, newVisitTemplateID, newVisitTagIDList;
    var newVisitFinalStatus, newExtraMinutes, newRecordedArrivalDateTime, newRecordedDepartureDateTime, newIsConfirmed, newIsBillableToClient, newIncludeInPayroll;
    ...
    newRecordedArrivalDateTime = neuro.cal.editvisit.getArrivalDepartureTimes(neuro.cal.editDialogRecordedArrivalTimeID);
    newRecordedDepartureDateTime = neuro.cal.editvisit.getArrivalDepartureTimes(neuro.cal.editDialogRecordedDepartureTimeID);
...
// Same as above, except we also incorporate the user's drag or resize -- much less stuff is changed.

neuro.cal.editvisit.constructCalVisitJoinAfterDragOrResize = function (v, newVisitDateTime, newStaffID, newVisitTemplateID, newExtraMinutes) {
    'use strict';
...
neuro.cal.editvisit.updateVisitJoinWS = function (calVisitJoin, updateAllFuture, updateScheduleOnly) {
    'use strict';
...
neuro.cal.editvisit.deleteVisitJoinWS = function (visitID, updateAllFuture) {
    ...
    calVisitJoin.FakeVisitID = '';
    calVisitJoin.ScheduleDate = origDateUTC;            // Date of the edited visit
...
neuro.cal.editvisit.WSSuccess = function (returnedValue, context, methodName) {
    ...
    var calMessagingOpenPopupJavaScriptCode = new AdaCareWeb.WebServices.CalMessagingOpenPopupJavaScriptCode();
    ...
    var $newElem;
    var telephonyID = '';
    var address = '', cityState = '', resPhone = '', mobilePhone = '', emergencyPhone = '', emailAddress = '', emailAddressUrl = '', contactInfoHtml = '', schedulingPrefsHtml = '', billingMinutesToString = '', payrollMinutesToString = '';
    var groupNum;
    ...
    switch (methodName) {
        ...
        case 'ReloadVisitJoin':
            ...
            if (returnedValue !== null) {
                ...
                $newElem = neuro.cal.constructSingleVisitElem(v, visitOflowID, v.visitID);
                neuro.cal.$body.append($newElem);
                neuro.cal.placeVisit(v, $newElem, false);
                neuro.cal.enableWidgetsForOneVisit(visitID);
            }
        ...
        case 'FetchClient':
            ...
            if (returnedValue !== null) {
                ...
                if (calClient.ID !== -1) {
                    ...
                    emergencyPhone = adacare.lib.stringTrim(calClient.EmergencyPhone);
                    ...
                    schedulingPrefsHtml = (calClient.SchedulingPrefsHtml ? adacare.lib.stringTrim(calClient.SchedulingPrefsHtml) : '');
            ...
            $('#' + neuro.cal.clientOrStaffDetail.emergencyPhoneID).text(emergencyPhone);
            ...
            $('#' + neuro.cal.clientOrStaffDetail.contactInfoID).html(contactInfoHtml);
            $('#' + neuro.cal.clientOrStaffDetail.schedulingPrefsID).html(schedulingPrefsHtml);
        ...
        case 'FetchStaff':
            ...
            if (returnedValue !== null) {
                ...
                if (calStaff.ID !== -1) {
                    ...
                    emergencyPhone = adacare.lib.stringTrim(calStaff.EmergencyPhone);
                    ...
                    schedulingPrefsHtml = (calStaff.SchedulingPrefsHtml ? adacare.lib.stringTrim(calStaff.SchedulingPrefsHtml) : '');
                }
        ...
        case 'MessagingOpenPopupJavaScriptCode':

            if (returnedValue !== null) {
                calMessagingOpenPopupJavaScriptCode = returnedValue;

                neuro.cal.setMessagingPopupButtonHandler(neuro.cal.editDialogClientOpenMessagingPopupButtonSurroundID, calMessagingOpenPopupJavaScriptCode.ClientMessagingOpenPopupJavaScriptCode);
                neuro.cal.setMessagingPopupButtonHandler(neuro.cal.editDialogStaffOpenMessagingPopupButtonSurroundID, calMessagingOpenPopupJavaScriptCode.StaffMessagingOpenPopupJavaScriptCode);
            }
            break;
...
neuro.cal.editvisit.displayClientForEditWS = function (calClient) {
    ...
    var address = '', cityState = '', resPhone = '', mobilePhone = '', emergencyPhone = '', emailAddress = '', emailAddressUrl = '', contactInfoHtml = '', schedulingPrefsHtml = '', billingMinutesToString = '';
    ...
    $('#' + neuro.cal.editDialogClientEmergencyPhoneID).text(emergencyPhone);
...
neuro.cal.editvisit.displayStaffForEditWS = function (calStaff) {
    ...
    var address = '', cityState = '', resPhone = '', mobilePhone = '', emergencyPhone = '', contactInfoHtml = '', schedulingPrefsHtml = '', emailAddress = '', emailAddressUrl = '', payrollMinutesToString = '';
    var comboContactInfoAndSchedulingPrefsHtml = '';
    ...
    $('#' + neuro.cal.editDialogStaffEmergencyPhoneID).text(emergencyPhone);
...
/////////////////////////////////////////////////////////////////////////////////////////////////////
// Populate the ClientPayer selector with the client's list of ClientPayer records.

neuro.cal.loadClientPayerSelector = function (calVisitJoin) {
    'use strict';

    var $dropdownlist;
    var i;

    $dropdownlist = $('#' + neuro.cal.editDialogClientPayerID);
    $dropdownlist.empty();

    for (i = 0; i < neuro.cal.clientPayerInfoList.length; i++) {

        $("<option />", {
            val: String(neuro.cal.clientPayerInfoList[i].ID),
            text: String(neuro.cal.clientPayerInfoList[i].Descrip)
...
neuro.cal.editvisit.displayVisitJoinForEditWS = function (calVisitJoin) {
    ...
    var VSEValues = new adacare.vsSpecialRatesEditor.VSEValues();
    var v, visitID;
    var scheduledDepartureDateTimeLocal, recordedArrivalDateTimeLocal, recordedDepartureDateTimeLocal;
    ...
    if (calVisitJoin !== null) {
        ...
        // Remember the list of the clients ClientPayer records, and display them in the drop-down list.

        neuro.cal.clientPayerInfoList = calVisitJoin.ClientPayerInfoList;
        neuro.cal.loadClientPayerSelector(calVisitJoin);

        $('#' + neuro.cal.editDialogClientPayerID).val(calVisitJoin.ClientPayerID);
        neuro.cal.displayInsuranceAuthorizationDiv();
        $('#' + neuro.cal.editDialogContractID).val(calVisitJoin.ContractID); // Will this blow up if a contract isn't in the list?
        ...
        scheduledDepartureDateTimeLocal = calVisitJoin.ScheduledDepartureDateTimeStr;
        recordedArrivalDateTimeLocal = (calVisitJoin.RecordedArrivalDateTime ? new Date(calVisitJoin.RecordedArrivalDateTimeStr) : null);
        recordedDepartureDateTimeLocal = (calVisitJoin.RecordedDepartureDateTimeStr ? new Date(calVisitJoin.RecordedDepartureDateTimeStr) : null);

        neuro.cal.editvisit.initArrivalDepartureTimes(v, calVisitJoin.VisitFinalStatus, scheduledDepartureDateTimeLocal, recordedArrivalDateTimeLocal, recordedDepartureDateTimeLocal);
        neuro.cal.editvisit.showOrHideArrivalDepartureTimes();
...
/////////////////////////////////////////////////////////////////////////////////////////////////////
// Show/hide the "view authorization" div.

neuro.cal.displayInsuranceAuthorizationDiv = function () {
    'use strict';

    var clientPayerID;
    var i;

    // Look up the currently selected ClientPayer.
    ...
    for (i = 0; i < neuro.cal.clientPayerInfoList.length; i++) {
        ...
        if (neuro.cal.clientPayerInfoList[i].ID === clientPayerID) {
            ...
            // Turn on/off the div that displays the "view authorization" button.

            if (neuro.cal.clientPayerInfoList[i].RequiresAuthorization) {

                $('#' + neuro.cal.editvisit.InsuranceAuthorizationInfoPanelID).show();
                $('#' + neuro.cal.editvisit.ViewAuthorizationButtID).off('click').on('click', neuro.cal.handleViewAuthorizationButtonHandler); // 2018-11-27 SG: Added off() before on()
                //$('#' + neuro.cal.editvisit.ViewAuthorizationButtID).on('click', neuro.cal.handleViewAuthorizationButtonHandler); // 2018-11-27 SG: DELETE AFTER TEST
            }
            else {

                $('#' + neuro.cal.editvisit.InsuranceAuthorizationInfoPanelID).hide();
                $('#' + neuro.cal.editvisit.ViewAuthorizationButtID).off('click');
            }
...
// Handle clicks on the "view authorization" button.

neuro.cal.handleViewAuthorizationButtonHandler = function () {
    'use strict';

    neuro.cal.editvisit.fetchInsuranceAuthorizationWS();
};

neuro.cal.editvisit.fetchInsuranceAuthorizationWS = function () {
    'use strict';

    var v, visitID, scheduleID, outcomeID, clientPayerID;
    var targetDateUTC;
    var calVisitJoin;
    ...
    //AdaCareWeb.WebServices.CalServices.FetchVisitInsuranceAuthorizationDisplayInfo(scheduleID, outcomeID, targetDateUTC, clientPayerID,
    AdaCareWeb.WebServices.CalServices.FetchVisitInsuranceAuthorizationDisplayInfo(calVisitJoin,
        neuro.cal.editvisit.fetchInsuranceAuthorizationWSSuccess,
        neuro.cal.editvisit.fetchInsuranceAuthorizationWSFailed,
        'neuro.cal.editvisit.fetchInsuranceAuthorizationWS');
};

neuro.cal.editvisit.fetchInsuranceAuthorizationWSSuccess = function (insuranceAuthorizationDisplayInfo) {
    'use strict';

    var $insAuthDialog;

    if (insuranceAuthorizationDisplayInfo !== null) {

        $insAuthDialog = $('#' + neuro.cal.InsuranceAuthorizationDisplayToolDialogID);

        $insAuthDialog.dialog({
            title: 'Insurance Authorization',
            //buttons: { 'Cancel': function () { $(this).dialog('destroy'); return false; } },
        ...
        adacare.insAuthDisplayInfoTools.displayAuthorizations(
            insuranceAuthorizationDisplayInfo,
            true, // We're editing a visit, not a schedule
            neuro.cal.InsuranceAuthorizationDisplayToolPlanNameFullID,
            neuro.cal.InsuranceAuthorizationDisplayToolProblemTextID,
            neuro.cal.InsuranceAuthorizationDisplayToolInfoBlockID);
    }

    adacare.lib.pleaseWaitPopup('close');
};

neuro.cal.editvisit.fetchInsuranceAuthorizationWSFailed = function (error, context) {
    'use strict';
...
neuro.cal.editvisit.fetchAllVisitDataForEditWS = function (v) {
    ...
    dateStr = $('#' + neuro.cal.editDialogDatePickerID).val();
    timeStr = $('#' + neuro.cal.editDialogTimePickerID).val();
    visitDateTime = adacare.lib.parseDateTimeLocalized(adacare.lib.countryAbbr, dateStr, timeStr);

    preloadedMessageTextForClient = neuro.cal.editvisit.preloadedMessageTextForClient(visitDateTime);
    preloadedMessageTextForStaff = neuro.cal.editvisit.preloadedMessageTextForStaff(visitDateTime);
    ...
    AdaCareWeb.WebServices.CalServices.FetchAllVisitData(neuro.cal.officeID, v.clientID, v.staffID, v.scheduleID, v.outcomeID, targetDateUTC,
        preloadedMessageTextForClient, preloadedMessageTextForStaff,
...
neuro.cal.editvisit.fetchAllVisitDataForEditWSSuccess = function (calAllVisitData) {
    ...
    if (calAllVisitData !== null) {
        ...
        neuro.cal.editvisit.calAllVisitDataOriginal = calAllVisitData; // Save the original so we can look for changes when we send the edits back to the server.
        ...
        neuro.cal.editDialogStaffSelectMenuUpdateRecent(calAllVisitData.CalRecentStaffIDList, calAllVisitData.CalPreferredStaffIDList);
    }
...
neuro.cal.editvisit.fetchAllVisitDataForEditWSFailed = function (error, context) {
    ...
    if (neuro.cal.HandleAuthenticationError(error)) {
        ...
        neuro.cal.editvisit.Button(neuro.cal.$editDialog, 0, 'cancel');
        adacare.lib.pleaseWaitPopup('close');
    }
    else {
...
neuro.cal.fetchStaffForEditWS = function (staffID, visitDateForPayroll) {
    ...
    var dateStr, timeStr, visitDateTime, preloadedMessageText;

    dateStr = $('#' + neuro.cal.editDialogDatePickerID).val();
    timeStr = $('#' + neuro.cal.editDialogTimePickerID).val();
    visitDateTime = adacare.lib.parseDateTimeLocalized(adacare.lib.countryAbbr, dateStr, timeStr);

    preloadedMessageText = neuro.cal.editvisit.preloadedMessageTextForStaff(visitDateTime);

    AdaCareWeb.WebServices.CalServices.FetchStaff(neuro.cal.officeID, staffID, visitDateForPayroll,
        preloadedMessageText, // Preloaded text for message box
        neuro.cal.fetchStaffForEditWSSuccess, neuro.cal.fetchStaffForEditWSFailed, 'neuro.cal.fetchStaffForEditWS');
};
...
/////////////////////////////////////////////////////////////////////////////////////////////////////
/////////////////////////////////////////////////////////////////////////////////////////////////////
// Web services for messaging.

neuro.cal.editvisit.fetchMessagingOpenPopupJavaScriptCodeWS = function (clientID, staffID, preloadedMessageTextForClient, preloadedMessageTextForStaff) {
    'use strict';

    AdaCareWeb.WebServices.CalServices.MessagingOpenPopupJavaScriptCode(
        clientID, staffID, preloadedMessageTextForClient, preloadedMessageTextForStaff,
        neuro.cal.editvisit.WSSuccess,
        neuro.cal.editvisit.WSFailed,
        'neuro.cal.fetchMessagingOpenPopupJavaScriptCodeWS',
        null);
};

// Reload the messaging pop-up text for both the client and the staff.

neuro.cal.editvisit.reloadMessagingOpenPopupJavaScriptCode = function (clientID, staffID) {
    'use strict';
    ...
    neuro.cal.editvisit.fetchMessagingOpenPopupJavaScriptCodeWS(clientID, staffID, preloadedMessageTextForClient, preloadedMessageTextForStaff);

};

// Return the text that's preloaded into the messaging pop-up dialog. The text can be different for
// clients vs. staff if we decide to later.

neuro.cal.editvisit.preloadedMessageTextForClient = function (visitDateTime) {
    'use strict';

    var preloadedMessageText;

    preloadedMessageText = 'Your visit on '
        + adacare.lib.formatDateLocalized(adacare.lib.countryAbbr, visitDateTime)
        + ' '
        + adacare.lib.FormatTimeLocalized(adacare.lib.countryAbbr, visitDateTime)
        + ':';

    return preloadedMessageText;
};

neuro.cal.editvisit.preloadedMessageTextForStaff = function (visitDateTime) {
    'use strict';
...
neuro.cal.editvisit.setMessagingPopupButtonPreloadedTextForBoth = function () {
    'use strict';
    ...
    neuro.cal.setMessagingPopupButtonHandler(neuro.cal.editDialogClientOpenMessagingPopupButtonSurroundID, preloadedMessageTextForClient);
    neuro.cal.setMessagingPopupButtonHandler(neuro.cal.editDialogStaffOpenMessagingPopupButtonSurroundID, preloadedMessageTextForStaff);
};
