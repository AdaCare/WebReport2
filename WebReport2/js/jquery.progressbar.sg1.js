            ...
            this.construct = function (arg1, arg2) {
                ...
                return this.each(function (child) {
                    ...
                    var t = setInterval(function () {
                        ...
                        //$bar.css("background-position", (((config.width * -1)) + (getPercentage(config) * pixels)) + 'px 50%'); // Original code
                        //$bar.css("background-position", (((config.width * -1)) + (Math.min(getPercentage(config), 100) * pixels)) + 'px 50%'); // SG - stop bar at 100%
                        $bar.css("background-position", (Math.min((config.width * -1) + (getPercentage(config) * pixels), config.width)) + 'px 50%'); // SG - stop bar at 100%
                        $bar.attr('title', getText(config));
                        $text.html(getText(config));
